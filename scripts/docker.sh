...
case "$1" in
    build)
        docker build --platform linux/amd64 -t venus .
        ;;
    run)
        # Ensure the container is not already running
        if [ "$(docker ps -q -f name=venus)" ]; then
            echo "Container 'venus' is already running. Stopping it first."
            docker stop venus
            docker rm venus
        fi
    ...
    bash)
        ...
        if [ "$(docker ps -q -f name=venus)" ]; then
            docker exec -it venus bash
        else
            echo "Container 'venus' is not running. Please start it first."
            exit 1
        fi
        ;;
    *)
