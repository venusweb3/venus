...
venus is an open-source AI agent.
            ...
            "Use Existing")
                ...
                case "$action" in
                    ...
                    "Run")
                        ...
                        echo "$selected_names" > /tmp/venus_selected_names
                        ...
                        done < /tmp/venus_selected_names
                        rm -f /tmp/venus_selected_names
...
start_venus() {
    ...
    if ! kill -0 $SERVER_PID 2>/dev/null; then
        log_error "Failed to start venus server"
        return 1
    fi
    ...
    if ! kill -0 $CLIENT_PID 2>/dev/null; then
        log_error "Failed to start venus client"
        kill $SERVER_PID 2>/dev/null
        return 1
    fi

    # Open in browser
    log_info "Opening venus in your browser..."
    ...
    log_success "venus is now running"
    log_info "Press Ctrl+C to stop venus"
                ...
                case "$action" in
                    ...
                    "Update")
                        ...
                        if [ "$LOCAL" = "$REMOTE" ]; then
                            ...
                            # Ask to start
                            if gum confirm "Would you like to start venus now?"; then
                                start_venus
                                exit 0
                            fi
                        ...
                        log_info "Updates available. Updating venus..."
...
main() {
    ...
    if ! gum confirm "Ready to install venus?"; then
    ...
    log_info "venus has been stopped"
}
