...
export const depositBalance = async (
    ...
    return await sdk.escrow.depositBalance({
        ...
        onFailureCallback: (error) => {
            venusLogger.error("Deposit failed: ", error);
            throw error;
        },
    });
};
...
export const withdrawBalance = async (
    ...
    return await sdk.escrow.withdrawBalance({
        ...
        amount,
        onFailureCallback: (error) => {
            venusLogger.error("Withdrawal failed:", error);
            throw error;
        },
    });
};
...
export const startDeployment = async (
    ...
    if (isReady) {
        venusLogger.log("Deployment ready");
    } else {
        venusLogger.error(`Deployment not ready after ${maxAttempts} attempts`);
...
export const createOrder = async (
    ...
    venusLogger.debug("Creating order with iclYaml:", iclYaml);
    const sdk = await getSDKInstance(runtime);
    const config = await validateSpheronConfig(runtime);
...
export const getDeployment = async (
    ...
    venusLogger.debug("Getting deployment with lease ID:", leaseId);
    const sdk = await getSDKInstance(runtime);
    const config = await validateSpheronConfig(runtime);
