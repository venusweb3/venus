...
function isDeploymentContent(content: any): content is DeploymentContent {
    venusLogger.debug("Content for deployment operation:", content);
    if (
...
export default {
    ...
    handler: async (
        ...
        venusLogger.log("Starting DEPLOYMENT_OPERATION handler...");
        ...
        try {
            ...
            venusLogger.error("Deployment operation failed:", error.message);
            callback?.({
