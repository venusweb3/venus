...
import { getActorDetails } from "@venusos/core";
import {
    Action,
    ActionExample,
    Content,
...
const summarizeAction: Action = {
    ...
    handler: async (
        ...
        if (!dateRange) {
            venusLogger.error("Couldn't determine date range from message");
            callbackData.text =
...
Here is a detailed summary of the conversation between ${userName} and ${runtime.character.name}:\n\n${currentSummary.trim()}`;
            ...
            if (summaryContent.length > 1000) {
                ...
                venusLogger.debug("Uploading summary file to Slack...");
                ...
                try {
                    ...
                    venusLogger.error("Error uploading summary file:", error);
            ...
            venusLogger.error("Error in summary generation:", error);
            callbackData.text =
