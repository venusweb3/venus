...
export default {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        venusLogger.log("🔄 Validating Giphy configuration...");
        try {
    ...
    handler: async (
        ...
        venusLogger.log("🚀 Starting Giphy SEND_GIF handler...");

        if (!state) {
            venusLogger.log("Creating new state...");
            state = (await runtime.composeState(message)) as State;
        } else {
            venusLogger.log("Updating existing state...");
            state = await runtime.updateRecentMessageState(state);
        }

        try {
            venusLogger.log("Composing gif trigger context...");
            const gifContext = composeContext({
                state,
                template: sendGifTemplate,
            });

            venusLogger.log("Generating content from context...");
            ...
            if (!content.trigger || !content.searchTerm) {
                venusLogger.log("No gif triggered for this message.");
                return false;
            }
            ...
            if (callback) {
                ...
                venusLogger.log("✅ Sending callback with gif url:", message);
                callback(message);
            }
