...
import { DatabaseAdapter } from "@venusos/core";
...
export class SupabaseDatabaseAdapter extends DatabaseAdapter {
    async getRoom(roomId: UUID): Promise<UUID | null> {
        ...
        if (error) {
            venusLogger.error(`Error getting room: ${error.message}`);
            return null;
        }
    ...
    async getParticipantUserState(
        ...
        if (error) {
            venusLogger.error("Error getting participant user state:", error);
            return null;
        }
    ...
    async setParticipantUserState(
        ...
        if (error) {
            venusLogger.error("Error setting participant user state:", error);
    ...
    async getMemoriesByRoomIds(params: {
        ...
        if (error) {
            venusLogger.error("Error retrieving memories by room IDs:", error);
            return [];
        }
    ...
    async createAccount(account: Account): Promise<boolean> {
        const { error } = await this.supabase
            ...
            .upsert([account]);
        if (error) {
            venusLogger.error(error.message);
            return false;
        }
        return true;
    }
    ...
    async getActorDetails(params: { roomId: UUID }): Promise<Actor[]> {
      ...
      `
            ...
            if (response.error) {
                venusLogger.error("Error!" + response.error);
                return [];
            }
            const { data } = response;
    ...
    async log(params: {
        ...
        if (error) {
            venusLogger.error("Error inserting log:", error);
            throw new Error(error.message);
        }
    }
    ...
    async getMemoryById(memoryId: UUID): Promise<Memory | null> {
        ...
        if (error) {
            venusLogger.error("Error retrieving memory by ID:", error);
            return null;
        }
    ...
    async addParticipant(userId: UUID, roomId: UUID): Promise<boolean> {
        ...
        if (error) {
            venusLogger.error(`Error adding participant: ${error.message}`);
            return false;
        }
        return true;
    }
    ...
    async removeParticipant(userId: UUID, roomId: UUID): Promise<boolean> {
        ...
        if (error) {
            venusLogger.error(`Error removing participant: ${error.message}`);
            return false;
        }
        return true;
    }
    ...
    async getCache(params: {
        ...
        if (error) {
            venusLogger.error("Error fetching cache:", error);
            return undefined;
        }
    ...
    async setCache(params: {
        ...
        if (error) {
            venusLogger.error("Error setting cache:", error);
            return false;
        }
    ...
    async createKnowledge(knowledge: RAGKnowledgeItem): Promise<void> {
        try {
            ...
            venusLogger.error(`Error creating knowledge ${knowledge.id}:`, {
                error,
