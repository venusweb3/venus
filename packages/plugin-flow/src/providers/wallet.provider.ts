...
export class FlowWalletProvider implements IFlowSigner, IFlowScriptExecutor {
    ...
    async syncAccountInfo() {
        ...
        venusLogger.debug("Flow account info synced", {
...
function getSignerAddress(runtime: IAgentRuntime): string {
    ...
    if (!signerAddr) {
        venusLogger.error("No signer address");
    ...
    get: async (
        ...
        try {
            ...
            if (!info || info?.address !== walletProvider.address) {
                venusLogger.error("Invalid account info");
                return null;
            }
            ...
            venusLogger.error("Error in Flow wallet provider:", error.message);
            return null;
        }
    },
};
