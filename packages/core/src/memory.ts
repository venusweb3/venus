...
export class MemoryManager implements IMemoryManager {
    ...
    async addEmbeddingToMemory(memory: Memory): Promise<Memory> {
        ...
        try {
            ...
            venusLogger.error("Failed to generate embedding:", error);
    ...
    async createMemory(memory: Memory, unique = false): Promise<void> {
        ...
        if (existingMessage) {
            venusLogger.debug("Memory already exists, skipping");
            return;
        }

        venusLogger.log("Creating Memory", memory.id, memory.content.text);
