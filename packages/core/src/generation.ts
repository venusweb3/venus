...
import { venusLogger } from "./index.ts";
import {
    models,
    getModelSettings,
...
export async function trimTokens(
    ...
    venusLogger.warn(`Unsupported tokenizer type: ${tokenizerType}`);
    return truncateTiktoken("gpt-4o", context, maxTokens);
}
...
async function truncateAuto(
    ...
    try {
        ...
        venusLogger.error("Error in trimTokens:", error);
...
async function getOnChainEternalAISystemPrompt(runtime: IAgentRuntime): Promise<string> | undefined {
    ...
    if (agentId && providerUrl && contractAddress) {
        ...
        try {
            ...
            if (result) {
                venusLogger.info('on-chain system-prompt response', result[0]);
                const value = result[0].toString().replace("0x", "");
                let content = Buffer.from(value, 'hex').toString('utf-8');
                venusLogger.info('on-chain system-prompt', content);
...
async function fetchEternalAISystemPrompt(runtime: IAgentRuntime, content: string): Promise<string> | undefined {
    ...
    if (containsSubstring) {
        ...
        venusLogger.info("fetch lightHouse", lightHouse)
        const responseLH = await fetch(lightHouse, {
            method: "GET",
        });
        venusLogger.info("fetch lightHouse resp", responseLH)
        if (responseLH.ok) {
            const data = await responseLH.text();
            return data;
        } else {
            const gcs = content.replace(IPFS, "https://cdn.eternalai.org/upload/")
            venusLogger.info("fetch gcs", gcs)
            const responseGCS = await fetch(gcs, {
                method: "GET",
            });
            venusLogger.info("fetch lightHouse gcs", responseGCS)
            if (responseGCS.ok) {
...
function getCloudflareGatewayBaseURL(runtime: IAgentRuntime, provider: string): string | undefined {
    ...
    venusLogger.debug("Cloudflare Gateway Configuration:", {
    ...
    if (!isCloudflareEnabled) {
        venusLogger.debug("Cloudflare Gateway is not enabled");
        return undefined;
    }

    if (!cloudflareAccountId) {
        venusLogger.warn("Cloudflare Gateway is enabled but CLOUDFLARE_AI_ACCOUNT_ID is not set");
        return undefined;
    }

    if (!cloudflareGatewayId) {
        venusLogger.warn("Cloudflare Gateway is enabled but CLOUDFLARE_AI_GATEWAY_ID is not set");
        return undefined;
    }

    const baseURL = `https://gateway.ai.cloudflare.com/v1/${cloudflareAccountId}/${cloudflareGatewayId}/${provider.toLowerCase()}`;
    venusLogger.info("Using Cloudflare Gateway:", {
        provider,
        baseURL,
...
export async function generateText({
    ...
    venusLogger.log("Generating text...");

    venusLogger.info("Generating text with options:", {
        modelProvider: runtime.modelProvider,
        model: modelClass,
        verifiableInference,
    });
    venusLogger.log("Using provider:", runtime.modelProvider);
    ...
    if (verifiableInference && runtime.verifiableInferenceAdapter) {
        ...
        try {
            ...
            venusLogger.log("Verifiable inference result:", result);
            // Verify the proof
            const isValid =
            ...
            venusLogger.error("Error in verifiable inference:", error);
            throw error;
        }
    }

    const provider = runtime.modelProvider;
    venusLogger.debug("Provider settings:", {
        provider,
        hasRuntime: !!runtime,
        runtimeSettings: {
    ...
    venusLogger.info("Selected model:", model);
    ...
    try {
        ...
        switch (provider) {
            ...
            case ModelProviderName.AKASH_CHAT_API: {
                venusLogger.debug("Initializing OpenAI model with Cloudflare check");
                const baseURL = getCloudflareGatewayBaseURL(runtime, 'openai') || endpoint;

                //venusLogger.debug("OpenAI baseURL result:", { baseURL });
            ...
            case ModelProviderName.ETERNALAI: {
                venusLogger.debug("Initializing EternalAI model.");
                const openai = createOpenAI({
                    apiKey,
                    baseURL: endpoint,
                ...
                try {
                    ...
                    if (!on_chain_system_prompt) {
                        venusLogger.error(new Error("invalid on_chain_system_prompt"))
                    } else {
                        system_prompt = on_chain_system_prompt
                        venusLogger.info("new on-chain system prompt", system_prompt)
                    }
                } catch (e) {
                    venusLogger.error(e)
                }
                ...
                response = openaiResponse;
                venusLogger.debug("Received response from EternalAI model.");
                break;
            }
            ...
            case ModelProviderName.GOOGLE: {
                ...
                venusLogger.debug("Received response from Google model.");
                break;
            }
            ...
            case ModelProviderName.MISTRAL: {
                ...
                venusLogger.debug("Received response from Mistral model.");
                break;
            }

            case ModelProviderName.ANTHROPIC: {
                venusLogger.debug("Initializing Anthropic model with Cloudflare check");
                const baseURL = getCloudflareGatewayBaseURL(runtime, 'anthropic') || "https://api.anthropic.com/v1";
                venusLogger.debug("Anthropic baseURL result:", { baseURL });
                ...
                venusLogger.debug("Received response from Anthropic model.");
                break;
            }

            case ModelProviderName.CLAUDE_VERTEX: {
                venusLogger.debug("Initializing Claude Vertex model.");
            ...
            case ModelProviderName.GROK: {
                venusLogger.debug("Initializing Grok model.");
                ...
                venusLogger.debug("Received response from Grok model.");
                break;
            }

            case ModelProviderName.GROQ: {
                venusLogger.debug("Initializing Groq model with Cloudflare check");
                const baseURL = getCloudflareGatewayBaseURL(runtime, 'groq');
                venusLogger.debug("Groq baseURL result:", { baseURL });
                ...
                venusLogger.debug("Received response from Groq model.");
                break;
            }
            ...
            case ModelProviderName.LLAMALOCAL: {
                ...
                venusLogger.debug("Received response from local Llama model.");
                break;
            }

            case ModelProviderName.REDPILL: {
                venusLogger.debug("Initializing RedPill model.");
                ...
                venusLogger.debug("Received response from redpill model.");
                break;
            }

            case ModelProviderName.OPENROUTER: {
                venusLogger.debug("Initializing OpenRouter model.");
                const serverUrl = getEndpoint(provider);
                ...
                venusLogger.debug("Received response from OpenRouter model.");
                break;
            }

            case ModelProviderName.OLLAMA:
                {
                    venusLogger.debug("Initializing Ollama model.");
                ...
                venusLogger.debug("Received response from Ollama model.");
                break;

            case ModelProviderName.HEURIST: {
                venusLogger.debug("Initializing Heurist model.");
                ...
                venusLogger.debug("Received response from Heurist model.");
                break;
            }
            case ModelProviderName.GAIANET: {
                venusLogger.debug("Initializing GAIANET model.");
                ...
                venusLogger.debug("Using GAIANET model with baseURL:", baseURL);
                ...
                response = openaiResponse;
                venusLogger.debug("Received response from GAIANET model.");
                break;
            }

            case ModelProviderName.GALADRIEL: {
                venusLogger.debug("Initializing Galadriel model.");
                const headers = {};
                ...
                venusLogger.debug("Received response from Galadriel model.");
                break;
            }

            case ModelProviderName.INFERA: {
                venusLogger.debug("Initializing Infera model.");
                ...
                venusLogger.debug("Received response from Infera model.");
                break;
            }

            case ModelProviderName.VENICE: {
                venusLogger.debug("Initializing Venice model.");
                ...
                venusLogger.debug("Received response from Venice model.");
                break;
            }

            case ModelProviderName.DEEPSEEK: {
                venusLogger.debug("Initializing Deepseek model.");
                ...
                venusLogger.debug("Received response from Deepseek model.");
                break;
            }
        ...
        venusLogger.error("Error in generateText:", error);
        throw error;
    }
}
...
export async function generateShouldRespond({
    ...
    while (true) {
        try {
            ...
            venusLogger.debug("Received response from generateText:", response);
            const parsedResponse = parseShouldRespondFromText(response.trim());
            if (parsedResponse) {
                venusLogger.debug("Parsed response:", parsedResponse);
                return parsedResponse;
            } else {
                venusLogger.debug("generateShouldRespond no response");
            }
        } catch (error) {
            venusLogger.error("Error in generateShouldRespond:", error);
            if (
        ...
        venusLogger.log(`Retrying in ${retryDelay}ms...`);
...
export async function generateTrueOrFalse({
    ...
    while (true) {
        try {
            ...
            venusLogger.error("Error in generateTrueOrFalse:", error);
        }
...
export async function generateTextArray({
    ...
    if (!context) {
        venusLogger.error("generateTextArray context is empty");
        return [];
    }
    let retryDelay = 1000;
    ...
    while (true) {
        try {
            ...
            venusLogger.error("Error in generateTextArray:", error);
        }
...
export async function generateObjectDeprecated({
    ...
    if (!context) {
        venusLogger.error("generateObjectDeprecated context is empty");
        return null;
    }
    let retryDelay = 1000;
    ...
    while (true) {
        try {
            ...
            venusLogger.error("Error in generateObject:", error);
        }
...
export async function generateObjectArray({
    ...
    if (!context) {
        venusLogger.error("generateObjectArray context is empty");
        return [];
    }
    let retryDelay = 1000;
...
export async function generateMessageResponse({
    ...
    context = await trimTokens(context, max_context_length, runtime);
    venusLogger.debug("Context:", context);
    let retryLength = 1000; // exponential backoff
    while (true) {
        try {
            venusLogger.log("Generating message response..");
            ...
            if (!parsedContent) {
                venusLogger.debug("parsedContent is null, retrying");
                continue;
            }

            return parsedContent;
        } catch (error) {
            venusLogger.error("ERROR:", error);
            // wait for 2 seconds
            retryLength *= 2;
            await new Promise((resolve) => setTimeout(resolve, retryLength));
            venusLogger.debug("Retrying...");
        }
    }
}
...
export const generateImage = async (
    ...
    venusLogger.info("Generating image with options:", {
        ...
        if (runtime.imageModelProvider === ModelProviderName.HEURIST) {
            ...
            const base64s = await Promise.all(
                togetherResponse.data.map(async (image) => {
                    if (!image.url) {
                        venusLogger.error("Missing URL in image data:", image);
            ...
            venusLogger.debug(`Generated ${base64s.length} images`);
            ...
            const result = await fal.subscribe(model, {
                ...
                onQueueUpdate: (update) => {
                    if (update.status === "IN_PROGRESS") {
                        venusLogger.info(update.logs.map((log) => log.message));
                    }
                },
            });
...
async function handleAnthropic({
    ...
    venusLogger.debug("Handling Anthropic request with Cloudflare check");
    const baseURL = getCloudflareGatewayBaseURL(runtime, 'anthropic');
    venusLogger.debug("Anthropic handleAnthropic baseURL:", { baseURL });
...
async function handleGroq({
    ...
    venusLogger.debug("Handling Groq request with Cloudflare check");
    const baseURL = getCloudflareGatewayBaseURL(runtime, 'groq');
    venusLogger.debug("Groq handleGroq baseURL:", { baseURL });
    ...
    while (true) {
        try {
            ...
            if (actions) {
                ...
                venusLogger.debug("generateTweetActions no valid response");
            }
        } catch (error) {
            venusLogger.error("Error in generateTweetActions:", error);
            if (
                error instanceof TypeError &&
                error.message.includes("queueTextCompletion")
            ) {
