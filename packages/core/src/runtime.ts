...
export class AgentRuntime implements IAgentRuntime {
    ...
    getService<T extends Service>(service: ServiceType): T | null {
        ...
        if (!serviceInstance) {
            venusLogger.error(`Service ${service} not found`);
            return null;
        }
    ...
    async registerService(service: Service): Promise<void> {
        ...
        venusLogger.log("Registering service:", serviceType);
        ...
        venusLogger.success(`Service ${serviceType} registered successfully`);
    }
    ...
    constructor(opts: {
        ...
        venusLogger.info("Initializing AgentRuntime with options:", {
        ...
        venusLogger.success(`Agent ID: ${this.agentId}`);
        ...
        venusLogger.info("Setting model provider...");
        venusLogger.info("Model Provider Selection:", {
        ...
        venusLogger.info("Selected model provider:", this.modelProvider);
        venusLogger.info(
        ...
        // venusLogger.info("Selected model provider:", this.modelProvider); duplicated log ln: 343
        venusLogger.info(
        ...
        if (!Object.values(ModelProviderName).includes(this.modelProvider)) {
            venusLogger.error("Invalid model provider:", this.modelProvider);
            venusLogger.error(
                "Available providers:",
        ...
        if (!this.serverUrl) {
            venusLogger.warn("No serverUrl provided, defaulting to localhost");
        }
    ...
    async stop() {
        venusLogger.debug("runtime::stop - character", this.character);
    ...
    registerAction(action: Action) {
        venusLogger.success(`Registering action: ${action.name}`);
    ...
    async processActions(
        ...
        for (const response of responses) {
            if (!response.content?.action) {
                venusLogger.warn("No action found in the response content.");
                continue;
            }
            ...
            venusLogger.success(`Normalized action: ${normalizedAction}`);
            ...
            if (!action) {
                venusLogger.info("Attempting to find action in similes.");
            ...
            if (!action.handler) {
                venusLogger.error(`Action ${action.name} has no handler.`);
                continue;
            }
    ...
    async evaluate(
        ...
        const evaluatorPromises = this.evaluators.map(
            async (evaluator: Evaluator) => {
                venusLogger.log("Evaluating", evaluator.name);
    ...
    async ensureUserExists(
        ...
        if (!account) {
            ...
            venusLogger.success(`User ${userName} created successfully.`);
        }
    }
    ...
    async ensureRoomExists(roomId: UUID) {
        ...
        if (!room) {
            ...
            venusLogger.log(`Room ${roomId} created successfully.`);
        }
    }
