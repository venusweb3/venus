...
import venusLogger from "./logger";
...
class LocalEmbeddingModelManager {
    ...
    private async getRootPath(): Promise<string> {
        ...
        return rootPath.includes("/venus/")
            ? rootPath.split("/venus/")[0] + "/venus/"
    ...
    private async initializeModel(): Promise<void> {
        ...
        try {
            ...
            venusLogger.debug("Initializing BGE embedding model...");
            ...
            venusLogger.debug("BGE model initialized successfully");
        } catch (error) {
            venusLogger.error("Failed to initialize BGE model:", error);
            throw error;
        }
    }
    ...
    public async generateEmbedding(input: string): Promise<number[]> {
        ...
        try {
            ...
            venusLogger.debug("Raw embedding from BGE:", {
            ...
            venusLogger.error("Embedding generation failed:", error);
            throw error;
        }
    }
