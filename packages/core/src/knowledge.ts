...
async function get(
    ...
    if (!message?.content?.text) {
        venusLogger.warn("Invalid message for knowledge query:", {
            message,
    ...
    venusLogger.debug("Knowledge query:", {
    ...
    if (!processed || processed.trim().length === 0) {
        venusLogger.warn("Empty processed text for knowledge query");
        return [];
    }
...
export function preprocess(content: string): string {
    venusLogger.debug("Preprocessing text:", {
        input: content,
