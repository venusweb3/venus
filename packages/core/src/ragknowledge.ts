...
export class RAGKnowledgeManager implements IRAGKnowledgeManager {
    ...
    private preprocess(content: string): string {
        if (!content || typeof content !== "string") {
            venusLogger.warn("Invalid input for preprocessing");
            return "";
        }
    ...
    async createKnowledge(item: RAGKnowledgeItem): Promise<void> {
        if (!item.content.text) {
            venusLogger.warn("Empty content in knowledge item");
            return;
        }
        ...
        try {
            ...
            venusLogger.error(`Error processing knowledge ${item.id}:`, error);
            throw error;
        }
    }
    ...
    async processFile(file: {
        ...
        const timeMarker = (label: string) => {
            ...
            venusLogger.info(`[Timing] ${label}: ${time.toFixed(2)}s`);
        };
        ...
        try {
            ...
            venusLogger.info(`Generated ${totalChunks} chunks`);
            ...
            venusLogger.error(`Error processing file ${file.path}:`, error);
            throw error;
        }
    }
}
