...
import venusLogger from "./logger.ts";
...
async function getRemoteEmbedding(
    ...
    try {
        ...
        if (!response.ok) {
            venusLogger.error("API Response:", await response.text()); // Debug log
            throw new Error(
        ...
        venusLogger.error("Full error details:", e);
        throw e;
    }
}
...
export async function embed(runtime: IAgentRuntime, input: string) {
    venusLogger.debug("Embedding request:", {
    ...
    if (!input || typeof input !== "string" || input.trim().length === 0) {
        venusLogger.warn("Invalid embedding input:", {
            input,
            type: typeof input,
            length: input?.length,
        });
    ...
    async function retrieveCachedEmbedding(
        ...
        if (!input) {
            venusLogger.log("No input to retrieve cached embedding for");
            return null;
        }
