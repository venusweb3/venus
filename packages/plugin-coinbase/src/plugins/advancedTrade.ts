...
const tradeProvider: Provider = {
    get: async (runtime: IAgentRuntime, _message: Memory) => {
        venusLogger.debug("Starting tradeProvider function");
        try {
            ...
            try {
                ...
                venusLogger.error("Error fetching accounts:", error);
                return [];
            }
            ...
            try {
                ...
                venusLogger.error("Error fetching products:", error);
                return [];
            }
            ...
            try {
                ...
                venusLogger.error("Error reading CSV file:", error);
                return [];
            }
            ...
            try {
                ...
                venusLogger.error("Error parsing CSV data:", error);
                return [];
            }
            ...
            venusLogger.error("Error in tradeProvider:", error);
            return [];
        }
    },
};
...
export async function appendTradeToCsv(tradeResult: any) {
    venusLogger.debug("Starting appendTradeToCsv function");
    try {
        const csvWriter = createArrayCsvWriter({
            path: tradeCsvFilePath,
        ...
        venusLogger.info("Trade result:", tradeResult);
        ...
        venusLogger.info("Formatted trade for CSV:", formattedTrade);
        await csvWriter.writeRecords([formattedTrade]);
        venusLogger.info("Trade written to CSV successfully");
    } catch (error) {
        venusLogger.error("Error writing trade to CSV:", error);
        // Log the actual error for debugging
        if (error instanceof Error) {
            venusLogger.error("Error details:", error.message);
        }
    }
}
...
async function hasEnoughBalance(
    ...
    venusLogger.debug("Starting hasEnoughBalance function");
    try {
        const response = await client.listAccounts({});
        const accounts = JSON.parse(response);
        venusLogger.info("Accounts:", accounts);
        ...
        if (!account) {
            venusLogger.error(`No ${checkCurrency} account found`);
            return false;
        }
        ...
        venusLogger.info(`Has sufficient balance: ${hasBalance}`);

        return hasBalance;
    } catch (error) {
        venusLogger.error("Balance check failed with error:", {
...
export const executeAdvancedTradeAction: Action = {
    ...
    handler: async (
        ...
        // Initialize client
        venusLogger.debug("Starting advanced trade client initialization");
        try {
            ...
            venusLogger.info("Advanced trade client initialized");
        } catch (error) {
            venusLogger.error("Client initialization failed:", error);
            callback(
                {
        ...
        let tradeDetails;
        venusLogger.debug("Starting trade details generation");
        try {
            ...
            venusLogger.info("Trade details generated:", tradeDetails.object);
        } catch (error) {
            venusLogger.error("Trade details generation failed:", error);
            callback(
                {
        ...
        if (!isAdvancedTradeContent(tradeDetails.object)) {
            venusLogger.error("Invalid trade content:", tradeDetails.object);
            callback(
                {
        ...
        venusLogger.debug("Starting order configuration");
        try {
            if (orderType === "MARKET") {
                orderConfiguration =
                    side === "BUY"
            ...
            venusLogger.error("Order configuration failed:", error);
            callback(
                {
                    text:
        ...
        try {
            venusLogger.debug("Executing the trade");
            if (
            ...
            venusLogger.info("Trade executed successfully:", order);
        } catch (error) {
            venusLogger.error("Trade execution failed:", error?.message);
            callback(
                {
        ...
        try {
            ...
            venusLogger.info("Trade logged to CSV");
        } catch (csvError) {
            venusLogger.warn("Failed to log trade to CSV:", csvError);
