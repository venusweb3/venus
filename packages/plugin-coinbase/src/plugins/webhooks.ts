...
export const webhookProvider: Provider = {
    get: async (runtime: IAgentRuntime, _message: Memory) => {
        venusLogger.debug("Starting webhookProvider.get function");
        try {
            Coinbase.configure({
                apiKeyName:
            ...
            venusLogger.info("Listing all webhooks:", resp.data);
            ...
            venusLogger.error("Error in webhookProvider:", error);
            return [];
        }
    },
};
...
export const createWebhookAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, _message: Memory) => {
        venusLogger.info("Validating runtime for CREATE_WEBHOOK...");
        return (
            !!(
    ...
    handler: async (
        ...
        venusLogger.debug("Starting CREATE_WEBHOOK handler...");
        ...
        try {
            ...
            venusLogger.info("Creating webhook with details:", {
                networkId,
                notificationUri,
                eventType,
                eventTypeFilter,
            ...
            venusLogger.info("Webhook appended to CSV successfully");
        } catch (error) {
            venusLogger.error("Error during webhook creation:", error);
            callback(
                {
