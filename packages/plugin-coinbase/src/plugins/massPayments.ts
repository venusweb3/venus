...
export const massPayoutProvider: Provider = {
    get: async (runtime: IAgentRuntime, _message: Memory) => {
        venusLogger.debug("Starting massPayoutProvider.get function");
        try {
            Coinbase.configure({
                apiKeyName:
            ...
            venusLogger.info("Reading CSV file from:", csvFilePath);
            ...
            if (!fs.existsSync(csvFilePath)) {
                ...
                venusLogger.info("New CSV file created with headers.");
            }
            ...
            venusLogger.info("Parsed CSV records:", records);
            venusLogger.info("Current Balances:", balances);
            venusLogger.info("Last Transactions:", transactions);
            ...
            venusLogger.error("Error in massPayoutProvider:", error);
...
async function executeMassPayout(
    ...
    venusLogger.debug("Starting executeMassPayout function");
    const transactions: Transaction[] = [];
    const assetIdLowercase = assetId.toLowerCase();
    let sendingWallet: Wallet;
    try {
        venusLogger.debug("Initializing sending wallet");
        sendingWallet = await initializeWallet(runtime, networkId);
    } catch (error) {
        venusLogger.error("Error initializing sending wallet:", error);
        throw error;
    }
    for (const address of receivingAddresses) {
        venusLogger.info("Processing payout for address:", address);
        if (address) {
            try {
                ...
                venusLogger.info("Wallet balance for asset:", {
                    assetId,
                    walletBalance,
                });
                ...
                if (walletBalance.lessThan(transferAmount)) {
                    ...
                    venusLogger.error(insufficientFunds);
            ...
            venusLogger.info("Skipping invalid or empty address.");
            transactions.push({
    ...
    try {
        venusLogger.debug("Sending 1% to charity:", charityAddress);
        ...
        venusLogger.error("Error during charity transfer:", error);
        transactions.push({
            address: charityAddress,
    ...
    venusLogger.info("Finished processing mass payouts.");
    return transactions;
}
...
export const sendMassPayoutAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, _message: Memory) => {
        venusLogger.info("Validating runtime and message...");
        return (
            !!(
    ...
    handler: async (
        ...
        venusLogger.debug("Starting SEND_MASS_PAYOUT handler...");
        try {
            Coinbase.configure({
                apiKeyName:
                    runtime.getSetting("COINBASE_API_KEY") ??
            ...
            if (
                ...
                venusLogger.error("Missing or invalid input parameters:", {
                    network,
                    receivingAddresses,
                    transferAmount,
                    assetId,
...
- Asset ID is valid.`,
            ...
            venusLogger.info("◎ Starting mass payout...");
...
Check the CSV file for full details.`,
            ...
            venusLogger.error("Error during mass payouts:", error);
            callback(
