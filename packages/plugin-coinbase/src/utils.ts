...
import { venusLogger, IAgentRuntime, settings } from "@venusos/core";
import fs from "fs";
    ...
    if (!storedSeed || !storedWalletId) {
        ...
        try {
            ...
            if (walletIDSave && seedSave) {
                venusLogger.log("Successfully updated character secrets.");
            } else {
            ...
            venusLogger.error("Error updating character secrets:", error);
            throw error;
        }

        // Logging wallet creation
        venusLogger.log("Created and stored new wallet:", walletAddress);
    } else {
        ...
        venusLogger.log("Imported existing wallet for network:", networkId);
...
export async function executeTradeAndCharityTransfer(
    ...
    venusLogger.log("Wallet initialized:", {
        network,
    ...
    if (charityAddress && charityAmount > 0) {
        ...
        venusLogger.log("Charity Transfer successful:", {
    ...
    venusLogger.log("Trade initiated:", trade.toString());
    await trade.wait();
    venusLogger.log("Trade completed successfully:", trade.toString());
...
export async function appendTradeToCsv(trade: Trade) {
    try {
        ...
        venusLogger.log("Writing trade to CSV:", formattedTrade);
        await csvWriter.writeRecords([formattedTrade]);
        venusLogger.log("Trade written to CSV successfully.");
    } catch (error) {
...
export async function appendTransactionsToCsv(transactions: Transaction[]) {
    try {
        ...
        venusLogger.log("Writing transactions to CSV:", formattedTransactions);
        await csvWriter.writeRecords(formattedTransactions);
        venusLogger.log("All transactions written to CSV successfully.");
    } catch (error) {
        venusLogger.error("Error writing transactions to CSV:", error);
    }
}
...
export async function appendWebhooksToCsv(webhooks: Webhook[]) {
    try {
        ...
        if (!fs.existsSync(webhookCsvFilePath)) {
            venusLogger.warn("CSV file not found. Creating a new one.");
            const csvWriter = createArrayCsvWriter({
            ...
            venusLogger.log("New CSV file created with headers.");
        }
        const csvWriter = createArrayCsvWriter({
            path: webhookCsvFilePath,
            header: [
        ...
        venusLogger.log("Writing webhooks to CSV:", formattedWebhooks);
        await csvWriter.writeRecords(formattedWebhooks);
        venusLogger.log("All webhooks written to CSV successfully.");
    } catch (error) {
        venusLogger.error("Error writing webhooks to CSV:", error);
    }
}
...
export async function updateCharacterSecrets(
    ...
    try {
        ...
        if (!fs.existsSync(characterFilePath)) {
            venusLogger.error("Character file not found:", characterFilePath);
            return false;
        }
...
export async function executeTransferAndCharityTransfer(
    ...
    venusLogger.log("Initiating transfer:", transferDetails);
    const transfer = await wallet.createTransfer(transferDetails);
    venusLogger.log("Transfer initiated:", transfer.toString());
    await transfer.wait();
...
export async function executeTransfer(
    ...
    try {
        ...
        venusLogger.error("Error executing transfer:", error);
    }
    return transfer;
}
