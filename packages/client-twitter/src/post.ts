...
import { postActionResponseFooter } from "@venusos/core";
import { generateTweetActions } from "@venusos/core";
import { IImageDescriptionService, ServiceType } from "@venusos/core";
...
import { State } from "@venusos/core";
import { ActionResponse } from "@venusos/core";
...
export class TwitterPostClient {
    ...
    constructor(client: ClientBase, runtime: IAgentRuntime) {
        ...
        venusLogger.log("Twitter Client Configuration:");
        venusLogger.log(`- Username: ${this.twitterUsername}`);
        venusLogger.log(
        ...
        if (targetUsers) {
            venusLogger.log(`- Target Users: ${targetUsers}`);
        }
    ...
    async start() {
        ...
        const generateNewTweetLoop = async () => {
            ...
            venusLogger.log(`Next tweet scheduled in ${randomMinutes} minutes`);
        };
        ...
        const processActionsLoop = async () => {
            ...
            while (!this.stopProcessingActions) {
                try {
                    ...
                    if (results) {
                        venusLogger.log(`Processed ${results.length} tweets`);
                        venusLogger.log(
        ...
        venusLogger.log("Tweet generation loop started");
    ...
    async sendStandardTweet(
        ...
        try {
            ...
            venusLogger.error("Error sending standard Tweet:", error);
            throw error;
        }
    }
    ...
    async postTweet(
        ...
        try {
            venusLogger.log(`Posting new tweet:\n`);
            ...
            venusLogger.error("Error sending tweet:", error);
        }
    }
    ...
    async generateNewTweet() {
        venusLogger.log("Generating new tweet");
        ...
        try {
            ...
            venusLogger.debug("generate post prompt:\n" + context);
            ...
            try {
                if (this.approvalRequired) {
                    ...
                    venusLogger.log("Tweet sent for approval");
                } else {
                    venusLogger.log(`Posting new tweet:\n ${cleanedContent}`);
                    this.postTweet(
                        this.runtime,
            ...
            venusLogger.error("Error generating new tweet:", error);
        }
    }
    ...
    private async generateTweetContent(
        ...
        venusLogger.debug("generate tweet content response:\n" + response);
        ...
        try {
            ...
            venusLogger.debug("Response is not JSON, treating as plain text");
        }
    ...
    private async processTweetActions() {
        if (this.isProcessing) {
            venusLogger.log("Already processing tweet actions, skipping");
            return null;
        }
        ...
        try {
            ...
            venusLogger.log("Processing tweet actions");
            ...
            venusLogger.error("Error in processTweetActions:", error);
            throw error;
        } finally {
    ...
    > {
        ...
        for (const timeline of timelines) {
            ...
            try {
                ...
                if (actionResponse.like) {
                    if (this.isDryRun) {
                        ...
                        try {
                            ...
                            venusLogger.log(`Liked tweet ${tweet.id}`);
                        } catch (error) {
                            venusLogger.error(
                ...
                if (actionResponse.retweet) {
                    if (this.isDryRun) {
                        ...
                        try {
                            ...
                            venusLogger.log(`Retweeted tweet ${tweet.id}`);
                        } catch (error) {
                            venusLogger.error(
                ...
                venusLogger.error(`Error processing tweet ${tweet.id}:`, error);
                continue;
            }
        }
    ...
    private async handleTextOnlyReply(
        ...
        try {
            ...
            const imageDescriptions = [];
            if (tweet.photos?.length > 0) {
                venusLogger.log("Processing images in tweet for context");
                for (const photo of tweet.photos) {
                    const description = await this.runtime
                        .getService<IImageDescriptionService>(
                            ServiceType.IMAGE_DESCRIPTION
            ...
            if (tweet.quotedStatusId) {
                try {
                    ...
                    venusLogger.error("Error fetching quoted tweet:", error);
                }
            }
            ...
            if (!replyText) {
                venusLogger.error("Failed to generate valid reply content");
                return;
            }
            ...
            venusLogger.debug("Final reply text to be sent:", replyText);
            ...
            if (result) {
                venusLogger.log("Successfully posted reply tweet");
                ...
                venusLogger.error("Tweet reply creation failed");
            }
        } catch (error) {
            venusLogger.error("Error in handleTextOnlyReply:", error);
        }
    }
    ...
    private async checkApprovalStatus(
        ...
        try {
            ...
            venusLogger.log(`channel ${JSON.stringify(channel)}`);

            if (!(channel instanceof TextChannel)) {
                venusLogger.error("Invalid approval channel");
                return "PENDING";
            }
            ...
            venusLogger.error("Error checking approval status:", error);
            return "PENDING";
        }
    }
    ...
    private async handlePendingTweet() {
        venusLogger.log("Checking Pending Tweets...");
        const pendingTweetsKey = `twitter/${this.client.profile.username}/pendingTweet`;
        const pendingTweets =
            (await this.runtime.cacheManager.get<PendingTweet[]>(
                pendingTweetsKey
        ...
        for (const pendingTweet of pendingTweets) {
            ...
            if (isExpired) {
                venusLogger.log("Pending tweet expired, cleaning up");
            ...
            if (approvalStatus === "APPROVED") {
                ...
                venusLogger.log("Tweet Rejected, Cleaning Up");
                await this.cleanupPendingTweet(pendingTweet.discordMessageId);
