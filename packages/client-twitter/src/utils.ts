...
export async function buildConversationThread(
    ...
    async function processThread(currentTweet: Tweet, depth: number = 0) {
        venusLogger.debug("Processing tweet:", {
            id: currentTweet.id,
        ...
        if (!currentTweet) {
            venusLogger.debug("No current tweet found for thread building");
            return;
        }

        // Stop if we've reached our reply limit
        if (depth >= maxReplies) {
            venusLogger.debug("Reached maximum reply depth", depth);
            return;
        }
        ...
        if (visited.has(currentTweet.id)) {
            venusLogger.debug("Already visited tweet:", currentTweet.id);
            return;
        }
        ...
        if (currentTweet.inReplyToStatusId) {
            ...
            try {
                ...
                if (parentTweet) {
                    venusLogger.debug("Found parent tweet:", {
                        id: parentTweet.id,
                ...
                venusLogger.error("Error fetching parent tweet:", {
...
export async function sendTweet(
    ...
    for (const chunk of tweetChunks) {
        ...
        if (tweetResult) {
            ...
            venusLogger.error("Error sending tweet chunk:", {
                chunk,
                response: body,
            });
        }
