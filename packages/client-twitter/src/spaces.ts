...
`,
        ...
        venusLogger.error("[generateFiller] Error generating filler:", err);
        return "";
    }
}
...
async function speakFiller(
    ...
    venusLogger.log(`[Space] Filler (${fillerType}) => ${text}`);
...
`,
        ...
        venusLogger.error("[generateTopicsIfEmpty] GPT error =>", err);
...
export class TwitterSpaceClient {
    ...
    public async startPeriodicSpaceCheck() {
        venusLogger.log("[Space] Starting periodic check routine...");
        ...
        const routine = async () => {
            try {
                ...
                venusLogger.error("[Space] Error in routine =>", error);
    ...
    private async shouldLaunchSpace(): Promise<boolean> {
        ...
        if (r > (this.decisionOptions.randomChance ?? 0.3)) {
            venusLogger.log("[Space] Random check => skip launching");
            return false;
        }
        // Business hours
        if (this.decisionOptions.businessHoursOnly) {
            ...
            if (hour < 9 || hour >= 17) {
                venusLogger.log("[Space] Out of business hours => skip");
                return false;
            }
        }
        // Interval
        ...
        if (this.lastSpaceEndedAt) {
            ...
            if (now - this.lastSpaceEndedAt < minIntervalMs) {
                venusLogger.log("[Space] Too soon since last space => skip");
                return false;
            }
        }

        venusLogger.log("[Space] Deciding to launch a new Space...");
        return true;
    }
    ...
    public async startSpace(config: SpaceConfig) {
        venusLogger.log("[Space] Starting a new Twitter Space...");
        ...
        try {
            ...
            if (this.decisionOptions.enableRecording) {
                venusLogger.log("[Space] Using RecordToDiskPlugin");
                this.currentSpace.use(new RecordToDiskPlugin());
            }

            if (this.decisionOptions.enableSttTts) {
                venusLogger.log("[Space] Using SttTtsPlugin");
            ...
            if (this.decisionOptions.enableIdleMonitor) {
                venusLogger.log("[Space] Using IdleMonitorPlugin");
            ...
            venusLogger.log(`[Space] Space started => ${spaceUrl}`);
            ...
            process.on("SIGINT", async () => {
                venusLogger.log("[Space] SIGINT => stopping space");
                await speakFiller(
                    this.client.runtime,
                    this.sttTtsPlugin,
                    "CLOSING"
            ...
            venusLogger.error("[Space] Error launching Space =>", error);
            this.isSpaceRunning = false;
            throw error;
        }
    }
    ...
    private async manageCurrentSpace() {
        ...
        try {
            ...
            venusLogger.error("[Space] Error in manageCurrentSpace =>", error);
        }
    }
    ...
    private async handleSpeakerRequest(req: SpeakerRequest) {
        ...
        if (janusSpeakers.length < (this.decisionOptions.maxSpeakers ?? 1)) {
            venusLogger.log(`[Space] Accepting speaker @${req.username} now`);
            await speakFiller(
                this.client.runtime,
                this.sttTtsPlugin,
                "PRE_ACCEPT"
    ...
    private async acceptSpeaker(req: SpeakerRequest) {
        ...
        try {
            ...
            venusLogger.log(`[Space] Speaker @${req.username} is now live`);
        } catch (err) {
            venusLogger.error(
    ...
    private async removeSpeaker(userId: string) {
        ...
        try {
            ...
            venusLogger.log(`[Space] Removed speaker userId=${userId}`);
        } catch (error) {
            venusLogger.error(
    ...
    public async stopSpace() {
        ...
        try {
            venusLogger.log("[Space] Stopping the current Space...");
            await this.currentSpace.stop();
        } catch (err) {
            venusLogger.error("[Space] Error stopping Space =>", err);
        } finally {
            this.isSpaceRunning = false;
            this.spaceId = undefined;
