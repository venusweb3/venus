...
export class ClientBase extends EventEmitter {
    ...
    async init() {
        ...
        if (cachedCookies) {
            venusLogger.info("Using cached cookies");
            await this.setCookiesFromArray(cachedCookies);
        }

        venusLogger.log("Waiting for Twitter login");
        while (retries > 0) {
            try {
                if (await this.twitterClient.isLoggedIn()) {
                    // cookies are valid, no login required
                    venusLogger.info("Successfully logged in.");
                    break;
                } else {
                    ...
                    if (await this.twitterClient.isLoggedIn()) {
                        ...
                        venusLogger.info("Successfully logged in.");
                        venusLogger.info("Caching cookies");
                ...
                venusLogger.error(`Login attempt failed: ${error.message}`);
            }
        ...
        if (this.profile) {
            venusLogger.log("Twitter user ID:", this.profile.id);
            venusLogger.log(
                "Twitter loaded:",
    ...
    async fetchOwnPosts(count: number): Promise<Tweet[]> {
        venusLogger.debug("fetching own posts");
    ...
    async fetchHomeTimeline(
        ...
        venusLogger.debug("fetching home timeline");
        const homeTimeline = following
            ? await this.twitterClient.fetchFollowingTimeline(count, [])
            : await this.twitterClient.fetchHomeTimeline(count, []);

        venusLogger.debug(homeTimeline, { depth: Infinity });
        ...
        //venusLogger.debug("process homeTimeline", processedTimeline);
        return processedTimeline;
    }

    async fetchTimelineForActions(count: number): Promise<Tweet[]> {
        venusLogger.debug("fetching timeline for actions");
        ...
        // Related issue: https://github.com/venusos/agent-twitter-client/issues/43
    }
    ...
    async fetchSearchTweets(
        ...
        try {
            ...
            try {
                ...
                venusLogger.error("Error fetching search tweets:", error);
                return { tweets: [] };
            }
        } catch (error) {
            venusLogger.error("Error fetching search tweets:", error);
    ...
    private async populateTimeline() {
        venusLogger.debug("populating timeline...");
        ...
        if (cachedTimeline) {
            ...
            if (someCachedTweetsExist) {
                ...
                for (const tweet of tweetsToSave) {
                    venusLogger.log("Saving Tweet", tweet.id);
                    ...
                    venusLogger.log("Creating memory for tweet", tweet.id);
    ...
    async saveRequestMessage(message: Memory, state: State) {
        if (message.content.text) {
            ...
            if (
                ...
                venusLogger.debug("Message already saved", recentMessage[0].id);
            } else {
                await this.runtime.messageManager.createMemory({
                    ...message,
                    embedding: getEmbeddingZeroVector(),
