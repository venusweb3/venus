...
import { ITranscriptionService, venusLogger } from "@venusos/core";
...
export class SttTtsPlugin implements Plugin {
    ...
    onAttach(_space: Space) {
        venusLogger.log("[SttTtsPlugin] onAttach => space was attached");
    }
    ...
    init(params: { space: Space; pluginConfig?: Record<string, any> }): void {
        ...
        venusLogger.log("[SttTtsPlugin] Plugin config =>", config);
    ...
    private async processTtsQueue(): Promise<void> {
        while (this.ttsQueue.length > 0) {
            ...
            try {
                ...
                venusLogger.error("[SttTtsPlugin] TTS streaming error =>", err);
            }
        }
    ...
    public setSystemPrompt(prompt: string) {
        ...
        venusLogger.log("[SttTtsPlugin] setSystemPrompt =>", prompt);
    }
    ...
    public setGptModel(model: string) {
        this.gptModel = model;
        venusLogger.log("[SttTtsPlugin] setGptModel =>", model);
    }
    ...
    public clearChatContext() {
        this.chatContext = [];
        venusLogger.log("[SttTtsPlugin] clearChatContext => done");
    }

    cleanup(): void {
        venusLogger.log("[SttTtsPlugin] cleanup => releasing resources");
