...
import { generateObject, composeContext, ModelClass } from "@venusos/core";
import {
    createPublicClient,
    createWalletClient,
    http,
...
async function ensureAllowance(
    ...
    venusLogger.log("allowance:", allowance);
    ...
    if (allowance < amount) {
        ...
        venusLogger.log(`Approving hash: ${hash}`);
        await publicClient.waitForTransactionReceipt({ hash });
        venusLogger.log(`Approving success: ${hash}`);
    } else {
        venusLogger.log(`No need to approve`);
    }
}
...
export const confiPump: Action = {
    ...
    handler: async (
        ...
        try {
            ...
            venusLogger.log("simulate: ", simulate);
            ...
            venusLogger.error(`Error performing the action: ${error}`);
            if (callback) {
                callback({
