...
export const executeSwapForDAO: Action = {
    ...
    handler: async (
        ...
        try {
            ...
            venusLogger.log("Swap Quote:", quoteData);

            const confirmSwap = await promptConfirmation();
            if (!confirmSwap) {
                venusLogger.log("Swap canceled by user");
                return false;
            }
            ...
            venusLogger.log("DAO Swap completed successfully!");
            venusLogger.log(`Transaction ID: ${txid}`);

            return true;
        } catch (error) {
            venusLogger.error("Error during DAO token swap:", error);
            return false;
        }
    },
    examples: [
