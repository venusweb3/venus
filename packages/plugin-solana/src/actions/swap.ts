...
async function swapToken(
    ...
    try {
        ...
        venusLogger.log("Decimals:", decimals.toString());
        ...
        venusLogger.log("Fetching quote with params:", {
        ...
        if (!quoteData || quoteData.error) {
            venusLogger.error("Quote error:", quoteData);
            throw new Error(
                `Failed to get quote: ${quoteData?.error || "Unknown error"}`
            );
        }

        venusLogger.log("Quote received:", quoteData);
        ...
        venusLogger.log("Requesting swap with body:", swapRequestBody);
        ...
        if (!swapData || !swapData.swapTransaction) {
            venusLogger.error("Swap error:", swapData);
            throw new Error(
                `Failed to get swap transaction: ${swapData?.error || "No swap transaction returned"}`
            );
        }

        venusLogger.log("Swap transaction received");
        return swapData;
    } catch (error) {
        venusLogger.error("Error in swapToken:", error);
        throw error;
    }
}
...
export const executeSwap: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        ...
        venusLogger.log("Message:", message);
        return true;
    },
    ...
    handler: async (
        ...
        venusLogger.log("Response:", response);
        ...
        if (!response.amount) {
            venusLogger.log("No amount provided, skipping swap");
            const responseMsg = {
        ...
        if (!response.amount) {
            venusLogger.log("Amount is not a number, skipping swap");
            const responseMsg = {
        ...
        try {
            ...
            venusLogger.log("Wallet Public Key:", walletPublicKey);
            venusLogger.log("inputTokenSymbol:", response.inputTokenCA);
            venusLogger.log("outputTokenSymbol:", response.outputTokenCA);
            venusLogger.log("amount:", response.amount);
            ...
            venusLogger.log("Deserializing transaction...");
            ...
            venusLogger.log("Preparing to sign transaction...");

            venusLogger.log("Creating keypair...");
            ...
            venusLogger.log("Signing transaction...");
            transaction.sign([keypair]);

            venusLogger.log("Sending transaction...");
            ...
            venusLogger.log("Transaction sent:", txid);
            ...
            venusLogger.log("Swap completed successfully!");
            venusLogger.log(`Transaction ID: ${txid}`);
            ...
            venusLogger.error("Error during token swap:", error);
            return false;
        }
    },
    examples: [
