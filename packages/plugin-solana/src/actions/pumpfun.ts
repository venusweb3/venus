...
import { generateImage } from "@venusos/core";
...
export function isCreateAndBuyContent(
    ...
    venusLogger.log("Content for create & buy", content);
    return (
...
export const createAndBuyToken = async ({
    ...
    venusLogger.log("Create Results: ", createResults);
    ...
    if (createResults.success) {
        ...
        if (amount === null) {
            ...
            venusLogger.log(`${deployer.publicKey.toBase58()}:`, amount);
        }
        ...
        venusLogger.log("Create and Buy failed");
        return {
            success: false,
            ca: mint.publicKey.toBase58(),
...
export const buyToken = async ({
    ...
    if (buyResults.success) {
        venusLogger.log("Success:", `https://pump.fun/${mint.toBase58()}`);
        const ata = getAssociatedTokenAddressSync(
            mint,
            buyer.publicKey,
            allowOffCurve
        ...
        if (amount === null) {
            ...
            venusLogger.log(`${buyer.publicKey.toBase58()}:`, amount);
        }
    } else {
        venusLogger.log("Buy failed");
    }
};
...
export const sellToken = async ({
    ...
    if (sellResults.success) {
        ...
        if (amount === null) {
            ...
            venusLogger.log(`${seller.publicKey.toBase58()}:`, amount);
        }
    } else {
        venusLogger.log("Sell failed");
    }
};
...
export default {
    ...
    handler: async (
        ...
        venusLogger.log("Starting CREATE_AND_BUY_TOKEN handler...");
            ...
            if (!tokenMetadata.file || tokenMetadata.file.length < 100) {  // Basic validation
                try {
                    ...
                    if (imageResult.success && imageResult.data && imageResult.data.length > 0) {
                        ...
                        venusLogger.error("Failed to generate image:", imageResult.error);
                        return false;
                    }
                } catch (error) {
                    venusLogger.error("Error generating image:", error);
                    return false;
                }
            } */
        ...
        venusLogger.log(`Base64 data saved to: ${outputPath}`);
        ...
        try {
            ...
            if (!createAndBuyConfirmation) {
                venusLogger.log("Create and buy token canceled by user");
                return false;
            }
            ...
            venusLogger.log("Executing create and buy transaction...");
            ...
            venusLogger.log(successMessage);
