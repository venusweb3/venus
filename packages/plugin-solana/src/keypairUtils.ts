...
import { DeriveKeyProvider, TEEMode } from "@venusos/plugin-tee";
...
export async function getWalletKey(
    ...
    if (requirePrivateKey) {
        ...
        try {
            ...
            venusLogger.log("Error decoding base58 private key:", e);
            try {
                // Then try base64
                venusLogger.log("Try decoding base64 instead");
                const secretKey = Uint8Array.from(
                    Buffer.from(privateKeyString, "base64")
                );
                return { keypair: Keypair.fromSecretKey(secretKey) };
            } catch (e2) {
                venusLogger.error("Error decoding private key: ", e2);
