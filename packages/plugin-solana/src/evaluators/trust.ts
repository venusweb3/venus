...
async function handler(runtime: IAgentRuntime, message: Memory) {
    venusLogger.log("Evaluating for trust");
    ...
    if (runtime.getSetting("POSTGRES_URL")) {
        venusLogger.warn("skipping trust evaluator because db is postgres");
        return [];
    }
    ...
    if (!shouldProcess) {
        venusLogger.log("Skipping process");
        return [];
    }

    venusLogger.log("Processing recommendations");
    ...
    venusLogger.log("recommendations", recommendations);
    ...
    for (const rec of filteredRecommendations) {
        ...
        if (!rec.contractAddress) {
            ...
            if (!tokenAddress) {
                ...
                rec.contractAddress = tokenAddress;
                if (!tokenAddress) {
                    venusLogger.warn("Could not find contract address for token");
                    continue;
                }
            }
        }
        ...
        if (!user) {
            venusLogger.warn("Could not find user: ", rec.recommender);
            continue;
        }
        ...
        venusLogger.log("recommendationsManager", rec);
        ...
        switch (rec.type) {
            ...
            case "dont_sell":
            case "dont_buy":
                venusLogger.warn("Not implemented");
                break;
        }
    }
