...
export class TokenProvider {
    ...
    private async fetchWithRetry(
        ...
        for (let i = 0; i < PROVIDER_CONFIG.MAX_RETRIES; i++) {
            try {
                ...
                venusLogger.error(`Attempt ${i + 1} failed:`, error);
                lastError = error as Error;
                if (i < PROVIDER_CONFIG.MAX_RETRIES - 1) {
                    const delay = PROVIDER_CONFIG.RETRY_DELAY * Math.pow(2, i);
                    venusLogger.log(`Waiting ${delay}ms before retrying...`);
    ...
    async getTokenFromWallet(runtime: IAgentRuntime, tokenSymbol: string) {
        try {
            ...
            venusLogger.error("Error checking token in wallet:", error);
            return null;
        }
    }
    ...
    async fetchPrices(): Promise<Prices> {
        try {
            ...
            if (cachedData) {
                venusLogger.log("Returning cached prices.");
                return cachedData;
            }
            ...
            venusLogger.error("Error fetching prices:", error);
            throw error;
        }
    }
    ...
    async fetchTokenSecurity(): Promise<TokenSecurityData> {
        ...
        venusLogger.log(`Token security data cached for ${this.tokenAddress}.`);
    ...
    async fetchTokenTradeData(): Promise<TokenTradeData> {
        ...
        const data = await fetch(url, options)
            .then((res) => res.json())
            .catch((err) => venusLogger.error(err));
    ...
    async fetchDexScreenerData(): Promise<DexScreenerData> {
        ...
        if (cachedData) {
            venusLogger.log("Returning cached DexScreener data.");
            return cachedData;
        }
        ...
        try {
            ...
            venusLogger.error(`Error fetching DexScreener data:`, error);
            return {
                schemaVersion: "1.0.0",
                pairs: [],
            };
    ...
    async searchDexScreenerData(
        ...
        const cachedData = await this.getCachedData<DexScreenerData>(cacheKey);
        if (cachedData) {
            venusLogger.log("Returning cached search DexScreener data.");
            return this.getHighestLiquidityPair(cachedData);
        }

        const url = `https://api.dexscreener.com/latest/dex/search?q=${symbol}`;
        try {
            venusLogger.log(`Fetching DexScreener data for symbol: ${symbol}`);
            const data = await fetch(url)
                .then((res) => res.json())
                .catch((err) => {
                    venusLogger.error(err);
    ...
    async fetchHolderList(): Promise<HolderData[]> {
        ...
        if (cachedData) {
            venusLogger.log("Returning cached holder list.");
            return cachedData;
        }
        ...
        try {
            while (true) {
                ...
                venusLogger.log(`Fetching holders - Page ${page}`);
                if (page > 2) {
                    break;
                }
                const response = await fetch(url, {
            ...
            venusLogger.log(`Total unique holders fetched: ${holders.length}`);
            ...
            venusLogger.error("Error fetching holder list from Helius:", error);
    ...
    async countHighSupplyHolders(
        ...
        try {
            ...
            venusLogger.error("Error counting high supply holders:", error);
            return 0;
        }
    }
    ...
    async getProcessedTokenData(): Promise<ProcessedTokenData> {
        try {
            ...
            // venusLogger.log("Processed token data:", processedData);
            return processedData;
        } catch (error) {
            venusLogger.error("Error processing token data:", error);
            throw error;
        }
    }
    ...
    formatTokenData(data: ProcessedTokenData): string {
        ...
        venusLogger.log("Formatted token data:", output);
        return output;
    }

    async getFormattedTokenReport(): Promise<string> {
        try {
            venusLogger.log("Generating formatted token report...");
            const processedData = await this.getProcessedTokenData();
            return this.formatTokenData(processedData);
        } catch (error) {
            venusLogger.error("Error generating token report:", error);
...
const tokenProvider: Provider = {
    get: async (
        ...
        try {
            ...
            venusLogger.error("Error fetching token data:", error);
            return "Unable to fetch token information. Please try again later.";
        }
    },
};
