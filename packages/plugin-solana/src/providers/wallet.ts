...
export class WalletProvider {
    ...
    async fetchPortfolioValueCodex(runtime): Promise<WalletPortfolio> {
        try {
            ...
            if (!data || data.length === 0) {
                venusLogger.error("No portfolio data available", data);
    ...
    private async getTokenAccounts(walletAddress: string) {
        try {
            ...
            venusLogger.error("Error fetching token accounts:", error);
            return [];
        }
    }
}
...
const walletProvider: Provider = {
    get: async (
        ...
        try {
            ...
            venusLogger.error("Error in wallet provider:", error);
            return null;
        }
    },
};
