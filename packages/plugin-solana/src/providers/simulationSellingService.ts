...
export class SimulationSellingService {
    ...
    private async initializeRabbitMQ(amqpUrl: string) {
        try {
            ...
            venusLogger.log("Connected to RabbitMQ");
            // Start consuming messages
            this.consumeMessages();
        } catch (error) {
            venusLogger.error("Failed to connect to RabbitMQ:", error);
        }
    }
    ...
    private async consumeMessages() {
        const queue = "process_venus_simulation";
        ...
        venusLogger.log(`Listening for messages on queue: ${queue}`);
    }
    ...
    private async processMessage(message: string) {
        try {
            ...
            venusLogger.error("Error processing message:", error);
        }
    }
    ...
    public async startService() {
        ...
        venusLogger.log("Starting SellingService...");
        await this.startListeners();
    }

    public async startListeners() {
        // scanning recommendations and selling
        venusLogger.log("Scanning for token performances...");
    ...
    private processTokenPerformances(tokenPerformances: TokenPerformance[]) {
        ...
        venusLogger.log("Deciding when to sell and how much...");
    ...
    private async startProcessInTheSonarBackend(
        ...
        try {
            ...
            const response = await fetch(
                `${this.sonarBe}/venusos-sol/startProcess`,
                {
                    method: "POST",
                    headers: {
            ...
            venusLogger.log("Received response:", result);
            venusLogger.log(`Sent message to process token ${tokenAddress}`);
    ...
    private stopProcessInTheSonarBackend(tokenAddress: string) {
        try {
            return fetch(`${this.sonarBe}/venusos-sol/stopProcess`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "x-api-key": `${this.sonarBeToken}`,
    ...
    async updateTradeInBe(
        ...
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                ...
                if (attempt < retries) {
                    venusLogger.log(`Retrying in ${delayMs} ms...`);
                    await this.delay(delayMs); // Wait for the specified delay before retrying
                } else {
                    venusLogger.error("All attempts failed.");
                }
            }
        }
    }
