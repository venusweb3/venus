...
export class Cache {
    ...
    constructor() {
        ...
        // Find the 'venus' folder in the filepath and adjust the cache directory path
        const venusIndex = __dirname.indexOf("venus");
        if (venusIndex !== -1) {
            const pathTovenus = __dirname.slice(0, venusIndex + 5); // include 'venus'
            this.cacheDir = path.join(pathTovenus, "cache");
        } else {
