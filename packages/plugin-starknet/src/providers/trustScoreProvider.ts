...
export class TrustScoreManager {
    ...
    async getRecommenderBalance(recommenderWallet: string): Promise<number> {
        try {
            ...
            venusLogger.error("Error fetching balance", error);
            return 0;
        }
    }
...
export const trustScoreProvider: Provider = {
    async get(
        ...
        try {
            ...
            if (!userId) {
                venusLogger.error("User ID is missing from the message");
                return "";
            }
            ...
            venusLogger.error("Error in trust score provider:", error.message);
