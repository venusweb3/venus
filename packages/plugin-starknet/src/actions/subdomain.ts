...
export default {
    ...
    handler: async (
        ...
        venusLogger.log("Starting CREATE_SUBDOMAIN handler...");
        ...
        venusLogger.debug("Transfer content:", content);

        // Validate transfer content
        if (!isSubdomainCreation(content)) {
            venusLogger.error("Invalid content for CREATE_SUBDOMAIN action.");
            if (callback) {
                callback({
        ...
        try {
            ...
            venusLogger.error("Error during subdomain transfer:", error);
            if (callback) {
                callback({
