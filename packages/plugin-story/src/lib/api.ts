...
export async function getResource(
    ...
    try {
        ...
        if (res.ok) {
            venusLogger.log("Response is ok");
            return res.json();
        } else {
            venusLogger.log("Response is not ok");
            venusLogger.log(JSON.stringify(res));
...
export async function listResource(
    ...
    try {
        ...
        venusLogger.log(`Calling Story API ${resourceName}`);
        venusLogger.log(`STORY_API_KEY: ${API_KEY}`);
        venusLogger.log(`API_URL: ${API_URL}`);
        venusLogger.log(`API_VERSION: ${API_VERSION}`);
        venusLogger.log(`_options: ${JSON.stringify(_options)}`);
        ...
        venusLogger.log("List resource Error");
        console.error(error);
    }
}
