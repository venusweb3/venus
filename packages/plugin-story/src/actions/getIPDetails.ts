...
class GetIPDetailsAction {
    async getIPDetails(
        ...
        venusLogger.log("Fetching from", `${API_URL}/${RESOURCE_TYPE.ASSET}`);
...
export const getIPDetailsAction = {
    ...
    handler: async (
        ...
        venusLogger.log("Starting GET_IP_DETAILS handler...");
        ...
        try {
            ...
            venusLogger.error("Error fetching IP details:", e.message);
            callback?.({
