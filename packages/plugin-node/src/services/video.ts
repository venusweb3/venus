...
export class VideoService extends Service implements IVideoService {
    ...
    public async downloadMedia(url: string): Promise<string> {
        ...
        try {
            ...
            venusLogger.log("Error downloading media:", error);
    ...
    public async downloadVideo(videoInfo: any): Promise<string> {
        ...
        try {
            ...
            venusLogger.log("Error downloading video:", error);
    ...
    private async processVideoFromUrl(
        ...
        if (cached) {
            venusLogger.log("Returning cached video file");
            return cached;
        }

        venusLogger.log("Cache miss, processing video");
        venusLogger.log("Fetching video info");
        const videoInfo = await this.fetchVideoInfo(url);
        venusLogger.log("Getting transcript");
    ...
    async fetchVideoInfo(url: string): Promise<any> {
        if (url.endsWith(".mp4") || url.includes(".mp4?")) {
            try {
                ...
                venusLogger.log("Error downloading MP4 file:", error);
        ...
        try {
            ...
            venusLogger.log("Error fetching video info:", error);
    ...
    private async getTranscript(
        ...
        try {
            ...
            if (videoInfo.subtitles && videoInfo.subtitles.en) {
                venusLogger.log("Manual subtitles found");
            ...
            if (
                ...
                venusLogger.log("Automatic captions found");
            ...
            if (
                ...
                venusLogger.log("Music video detected, no lyrics available");
            ...
            venusLogger.log("Error in getTranscript:", error);
            throw error;
        }
    }

    private async downloadCaption(url: string): Promise<string> {
        venusLogger.log("Downloading caption from:", url);
        const response = await fetch(url);
        if (!response.ok) {
            throw new Error(
    ...
    private parseCaption(captionContent: string): string {
        venusLogger.log("Parsing caption");
        try {
            ...
            if (jsonContent.events) {
                ...
                venusLogger.log("Unexpected caption format:", jsonContent);
                return "Error: Unable to parse captions";
            }
        } catch (error) {
            venusLogger.log("Error parsing caption:", error);
            return "Error: Unable to parse captions";
        }
    }
    ...
    async transcribeAudio(
        ...
        venusLogger.log("Preparing audio for transcription...");
        ...
        if (!fs.existsSync(mp3FilePath)) {
            if (fs.existsSync(mp4FilePath)) {
                venusLogger.log("MP4 file found. Converting to MP3...");
                await this.convertMp4ToMp3(mp4FilePath, mp3FilePath);
            } else {
                venusLogger.log("Downloading audio...");
                await this.downloadAudio(url, mp3FilePath);
            }
        }

        venusLogger.log(`Audio prepared at ${mp3FilePath}`);

        const audioBuffer = fs.readFileSync(mp3FilePath);
        venusLogger.log(`Audio file size: ${audioBuffer.length} bytes`);

        venusLogger.log("Starting transcription...");
    ...
    private async convertMp4ToMp3(
        ...
        return new Promise((resolve, reject) => {
            ffmpeg(inputPath)
                ...
                .on("end", () => {
                    venusLogger.log("Conversion to MP3 complete");
                    resolve();
                })
                .on("error", (err) => {
                    venusLogger.log("Error converting to MP3:", err);
                    reject(err);
                })
                .run();
        });
    ...
    private async downloadAudio(
        ...
        try {
            ...
            venusLogger.log("Error downloading audio:", error);
