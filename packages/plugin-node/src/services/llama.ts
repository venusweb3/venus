...
import { Service } from "@venusos/core";
import fs from "fs";
...
export class LlamaService extends Service {
    ...
    async initialize(runtime: IAgentRuntime): Promise<void> {
        venusLogger.info("Initializing LlamaService...");
    ...
    private async ensureInitialized() {
        if (!this.modelInitialized) {
            ...
            venusLogger.info("Model already initialized");
        }
    }

    async initializeModel() {
        try {
            venusLogger.info("Checking model file...");
            ...
            venusLogger.info("Initializing Llama instance...");
            this.llama = await getLlama({
                gpu: hasCUDA ? "cuda" : undefined,
            });

            venusLogger.info("Creating JSON schema grammar...");
            ...
            venusLogger.info("Loading model...");
            this.model = await this.llama.loadModel({
                modelPath: this.modelPath,
            });

            venusLogger.info("Creating context and sequence...");
            this.ctx = await this.model.createContext({ contextSize: 8192 });
            this.sequence = this.ctx.getSequence();

            this.modelInitialized = true;
            venusLogger.success("Model initialization complete");
            this.processQueue();
        } catch (error) {
            venusLogger.error(
    ...
    async checkModel() {
        if (!fs.existsSync(this.modelPath)) {
            venusLogger.info("Model file not found, starting download...");
                    ...
                    https
                        .get(url, (response) => {
                            ...
                            response.on("data", (chunk) => {
                                ...
                                venusLogger.progress(progressString);
                            });

                            file.on("finish", () => {
                                file.close();
                                venusLogger.progress(""); // Clear the progress line
                                venusLogger.success("Model download complete");
                                resolve();
                            });
            ...
            venusLogger.warn("Model already exists.");
        }
    }
    ...
    async completion(prompt: string, runtime: IAgentRuntime): Promise<string> {
        try {
            ...
            venusLogger.error("Error in completion:", error);
            throw error;
        }
    }
    ...
    async embedding(text: string, runtime: IAgentRuntime): Promise<number[]> {
        try {
            ...
            venusLogger.error("Error in embedding:", error);
            throw error;
        }
    }
    ...
    private async getCompletionResponse(
        ...
        if (useGrammar) {
            ...
            try {
                ...
                venusLogger.error("Error parsing JSON:", error);
            }
        } else {
            await this.sequence.clearHistory();
            return response;
    ...
    private async localCompletion(prompt: string): Promise<string> {
        ...
        for await (const token of this.sequence.evaluate(tokens, {
            ...
            if (current.includes("\n")) {
                venusLogger.info("Stop sequence found");
                break;
            }
            ...
            if (responseTokens.length > 256) {
                venusLogger.info("Max tokens reached");
                break;
            }
        }
