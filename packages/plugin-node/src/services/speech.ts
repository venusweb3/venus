...
import { IAgentRuntime, ISpeechService, ServiceType } from "@venusos/core";
...
async function getVoiceSettings(runtime: IAgentRuntime) {
    ...
    venusLogger.debug("Voice settings:", {
        hasElevenLabs,
        useVits,
        voiceSettings,
        elevenlabsSettings,
    ...
    try {
        ...
        if (error.message === "QUOTA_EXCEEDED") {
            ...
            if (audio instanceof Buffer) {
                venusLogger.log("audio is a buffer");
                wavStream = Readable.from(audio);
            } else if ("audioChannels" in audio && "sampleRate" in audio) {
                venusLogger.log("audio is a RawAudio");
                const floatBuffer = Buffer.from(audio.audioChannels[0].buffer);
                venusLogger.log("buffer length: ", floatBuffer.length);
