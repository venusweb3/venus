...
import { Service, ServiceType } from "@venusos/core";
    ...
    implements ITranscriptionService
{
    ...
    private async convertAudio(inputBuffer: ArrayBuffer): Promise<Buffer> {
        ...
        try {
            ...
            venusLogger.log("Input audio info:", stream);
            ...
            venusLogger.log("FFmpeg command:", ffmpegCommand);
            ...
            venusLogger.error("Error converting audio:", error);
            throw error;
        }
    }
    ...
    private async saveDebugAudio(audioBuffer: ArrayBuffer, prefix: string) {
        ...
        venusLogger.log(`Debug audio saved: ${filePath}`);
    }
    ...
    private async transcribeWithOpenAI(
        ...
        venusLogger.log("Transcribing audio with OpenAI...");
        ...
        try {
            ...
            venusLogger.log(`OpenAI speech to text result: "${trimmedResult}"`);
            ...
            if (error.response) {
                venusLogger.error("Response data:", error.response.data);
                venusLogger.error("Response status:", error.response.status);
                venusLogger.error("Response headers:", error.response.headers);
            } else if (error.request) {
                venusLogger.error("No response received:", error.request);
            } else {
                venusLogger.error("Error setting up request:", error.message);
            }
            return null;
        }
    }
    ...
    public async transcribeLocally(
        ...
        try {
            venusLogger.log("Transcribing audio locally...");
            ...
            venusLogger.debug(`Temporary WAV file created: ${tempWavFile}`);
            ...
            if (!output || output.length < 5) {
                venusLogger.log("Output is null or too short, returning null");
                return null;
            }
            return output;
        } catch (error) {
