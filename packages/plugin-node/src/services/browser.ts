import { generateText, IBrowserService, trimTokens } from "@venusos/core";
import { parseJSONObjectFromText } from "@venusos/core";
import { Service } from "@venusos/core";
import { settings } from "@venusos/core";
import { IAgentRuntime, ModelClass, ServiceType } from "@venusos/core";
...
export class BrowserService extends Service implements IBrowserService {
    ...
    private async fetchPageContent(
        ...
        try {
            ...
            if (!response) {
                venusLogger.error("Failed to load the page");
            }
    ...
    private async solveCaptcha(page: Page, url: string): Promise<void> {
        try {
            ...
            venusLogger.error("Error solving CAPTCHA:", error);
        }
    }
    ...
    private async tryAlternativeSources(
        ...
        try {
            ...
            venusLogger.error("Error fetching from Internet Archive:", error);
        }
        ...
        try {
            ...
            venusLogger.error("Error fetching from Google Search:", error);
            venusLogger.error(
