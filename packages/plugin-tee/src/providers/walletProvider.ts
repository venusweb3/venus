...
export class WalletProvider {
    ...
    async fetchPortfolioValue(runtime): Promise<WalletPortfolio> {
        try {
            ...
            if (cachedValue) {
                venusLogger.log("Cache hit for fetchPortfolioValue");
                return cachedValue;
            }
            venusLogger.log("Cache miss for fetchPortfolioValue");
            ...
            if (!walletData?.success || !walletData?.data) {
                venusLogger.error("No portfolio data available", walletData);
            ...
            venusLogger.error("Error fetching portfolio:", error);
            throw error;
        }
    }
    ...
    async fetchPrices(runtime): Promise<Prices> {
        try {
            ...
            if (cachedValue) {
                venusLogger.log("Cache hit for fetchPrices");
                return cachedValue;
            }
            venusLogger.log("Cache miss for fetchPrices");
    ...
    async getFormattedPortfolio(runtime): Promise<string> {
        try {
            ...
            venusLogger.error("Error generating portfolio report:", error);
    ...
    get: async (
        ...
        try {
            ...
            try {
                ...
                venusLogger.log("Wallet Public Key: ", publicKey.toBase58());
            } catch (error) {
                venusLogger.error("Error creating PublicKey:", error);
                return "";
            }
            ...
            venusLogger.error("Error in wallet provider:", error.message);
