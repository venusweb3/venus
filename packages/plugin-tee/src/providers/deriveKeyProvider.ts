...
class DeriveKeyProvider {
    ...
    private async generateDeriveKeyAttestation(
        ...
        venusLogger.log("Remote Attestation Quote generated successfully!");
        return quote;
    }
    ...
    async rawDeriveKey(
        ...
        try {
            ...
            venusLogger.log("Deriving Raw Key in TEE...");
            const derivedKey = await this.client.deriveKey(path, subject);

            venusLogger.log("Raw Key Derived Successfully!");
            return derivedKey;
        } catch (error) {
            venusLogger.error("Error deriving raw key:", error);
            throw error;
        }
    }
    ...
    async deriveEd25519Keypair(
        ...
        try {
            ...
            venusLogger.log("Deriving Key in TEE...");
            const derivedKey = await this.client.deriveKey(path, subject);
            ...
            venusLogger.log("Key Derived Successfully!");

            return { keypair, attestation };
        } catch (error) {
            venusLogger.error("Error deriving key:", error);
            throw error;
        }
    }
    ...
    async deriveEcdsaKeypair(
        ...
        try {
            ...
            venusLogger.log("Deriving ECDSA Key in TEE...");
            ...
            venusLogger.log("ECDSA Key Derived Successfully!");

            return { keypair, attestation };
        } catch (error) {
            venusLogger.error("Error deriving ecdsa key:", error);
            throw error;
        }
    }
}
...
const deriveKeyProvider: Provider = {
    get: async (runtime: IAgentRuntime, _message?: Memory, _state?: State) => {
        ...
        try {
            ...
            venusLogger.error("Error in derive key provider:", error.message);
