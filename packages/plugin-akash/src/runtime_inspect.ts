...
export function inspectRuntime(runtime: IAgentRuntime) {
    ...
    if (hasPlugins) {
        ...
        venusLogger.info("Akash plugin status:", {
    ...
    if (hasActions) {
        ...
        venusLogger.info("Akash actions status:", {
...
export function isPluginLoaded(runtime: IAgentRuntime, pluginName: string): boolean {
    ...
    const plugins = (runtime as any).plugins as Plugin[];
    if (!plugins) {
        venusLogger.warn(`No plugins array found in runtime`);
        return false;
    }
    ...
    if (!plugin) {
        venusLogger.warn(`Plugin ${pluginName} not found in runtime plugins`);
        return false;
    }
    ...
    if (!actions || !actions.length) {
        venusLogger.warn(`No actions found in runtime`);
        return false;
    }
    ...
    if (!registeredActions) {
        venusLogger.warn(`Not all ${pluginName} actions are registered in runtime`);
        return false;
    }

    venusLogger.info(`Plugin ${pluginName} is properly loaded and registered`);
    return true;
}