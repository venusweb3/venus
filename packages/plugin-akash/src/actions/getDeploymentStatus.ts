...
export const getDeploymentStatusAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        venusLogger.debug("Validating get deployment status request", { message });
        try {
            ...
            if (params.text && !params.dseq) {
                ...
                if (dseqMatch) {
                    ...
                    venusLogger.debug("Extracted DSEQ from text", {
                        text: params.text,
            ...
            if (!params.dseq) {
                if (config.AKASH_DEP_STATUS === "dseq" && config.AKASH_DEP_DSEQ) {
                    ...
                    venusLogger.info("DSEQ parameter is required when AKASH_DEP_STATUS is set to param_passed", {
                    ...
                    venusLogger.info("No DSEQ provided and no valid environment configuration found", {
            ...
            venusLogger.error("Get deployment status validation failed", {
    ...
    handler: async (
        ...
        venusLogger.info("Starting deployment status request", { actionId });
        ...
        try {
            ...
            // Query deployment
            venusLogger.info("Querying deployment status", {
                dseq: params.dseq,
            ...
            try {
                ...
                venusLogger.info("Deployment status retrieved successfully", {
                    dseq: params.dseq,
                    state: status.state,
                    owner: status.owner,
                    actionId
                ...
                if (callback) {
                    ...
                    venusLogger.info("=== Preparing callback response for deployment status ===", {
                        hasCallback: true,
                        actionId,
                        dseq: params.dseq
                    });
                ...
                if (dseqSource === "environment") {
                    venusLogger.warn("Failed to query deployment from environment configuration", {
                        dseq: params.dseq,
                    ...
                    venusLogger.error("Failed to query deployment", {
                        error: queryError instanceof Error ? queryError.message : String(queryError),
                        actionId
                    });
                    if (callback) {
            ...
            venusLogger.error("Get deployment status request failed", {
