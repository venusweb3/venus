...
function getTotalTokensFromString(str: string): number {
    try {
        ...
        venusLogger.warn("Failed to count tokens", { error });
        return 0;
    }
}
...
export const estimateGas: Action = {
  ...
  validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
    venusLogger.debug("Validating gas estimation request", { message });
    try {
      ...
      if (!params.operation) {
        if (config.AKASH_GAS_OPERATION) {
          ...
          venusLogger.info("Using operation from environment", { operation: params.operation });
        } else {
      ...
      if (params.operation === "close") {
        if (!params.dseq) {
          if (config.AKASH_GAS_DSEQ) {
            ...
            venusLogger.info("Using DSEQ from environment", { dseq: params.dseq });
          } else {
            throw new AkashError(
      ...
      venusLogger.error("Gas estimation validation failed", {
  ...
  handler: async (
    ...
    venusLogger.info("Starting gas estimation", { actionId });

    venusLogger.debug("=== Handler Parameters ===", {
        hasRuntime: !!runtime,
        hasMessage: !!message,
        hasState: !!state,
        hasOptions: !!options,
    ...
    try {
        ...
        // Estimate gas
        venusLogger.info("Estimating gas for operation", {
        ...
        venusLogger.info("Gas estimation completed", {
            gasEstimate,
            operation: params.operation,
            dseq: params.dseq,
            owner: account.address,
        ...
        if (callback) {
            venusLogger.info("=== Preparing callback response for gas estimation ===", {
                hasCallback: true,
                actionId,
                operation: params.operation,
                dseq: params.dseq
            ...
            venusLogger.warn("=== No callback provided for gas estimation ===", {
                actionId,
                operation: params.operation,
                dseq: params.dseq
            });
        ...
        venusLogger.error("Gas estimation failed", {
        ...
        if (callback) {
            ...
            venusLogger.warn("=== No callback provided for error handling ===", {
                actionId,
