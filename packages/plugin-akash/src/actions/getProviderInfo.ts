...
export const getProviderInfoAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        venusLogger.debug("Validating get provider info request", { message });
        try {
            ...
            if (params.text && !params.provider) {
                ...
                if (providerMatch) {
                    ...
                    venusLogger.debug("Extracted provider address from text", {
                        text: params.text,
            ...
            venusLogger.error("Get provider info validation failed", {
    ...
    handler: async (
        ...
        venusLogger.info("Starting provider info request", { actionId });
        ...
        try {
            ...
            venusLogger.info("Querying provider information", {
            ...
            try {
                ...
                venusLogger.info("Querying provider status", {
                    hostUri: response.provider.hostUri,
                    actionId
                });

                const hostUri = response.provider.hostUri.replace(/^https?:\/\//, '');
                venusLogger.debug("Making provider status request", { url: `https://${hostUri}/status` });
                ...
                try {
                    ...
                    if (!statusResponse.ok) {
                        venusLogger.debug("Provider status not available", {
                    ...
                    venusLogger.debug("Provider status fetch failed", {
                ...
                venusLogger.info("Provider information retrieved successfully", {
                    provider: params.provider,
                    hostUri: response.provider.hostUri,
                ...
                if (callback) {
                    venusLogger.info("=== Preparing callback response for provider info ===", {
                        hasCallback: true,
                        actionId,
                        provider: params.provider
                    });
            ...
            venusLogger.error("Get provider info request failed", {
