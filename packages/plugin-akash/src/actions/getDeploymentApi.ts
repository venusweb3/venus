...
async function fetchWithRetry(url: string, options: RequestInit, retries = 3, delay = 1000): Promise<Response> {
    for (let i = 0; i < retries; i++) {
        try {
            ...
            venusLogger.warn(`API request failed (attempt ${i + 1}/${retries})`, {
            ...
            venusLogger.warn(`API request error (attempt ${i + 1}/${retries})`, {
...
export async function initializeWallet(runtime: IAgentRuntime): Promise<{wallet: DirectSecp256k1HdWallet | null, address: string}> {
    try {
        ...
        venusLogger.info("Initializing wallet with config", {
        ...
        if (config.AKASH_WALLET_ADDRESS) {
            venusLogger.info("Using provided wallet address", {
        ...
        try {
            venusLogger.info("Creating wallet from mnemonic");
            ...
            venusLogger.info("Wallet initialized from mnemonic", {
                address,
...
export async function fetchDeployments(
    ...
    venusLogger.info("Initializing deployment fetch", {
        status: status || 'all',
        skip,
        limit
    });
    ...
    try {
        ...
        venusLogger.info("Fetching deployments from API", {
            address,
            status: status || 'all',
            skip,
            limit
        ...
        venusLogger.debug("Making API request", { url });
        ...
        venusLogger.info("Deployments fetched successfully", {
            count: data.count,
        ...
        venusLogger.info("Using deployments directory", { deploymentDir });

        // Create base deployments directory if it doesn't exist
        if (!fs.existsSync(deploymentDir)) {
            venusLogger.info("Creating deployments directory", { deploymentDir });
        ...
        for (const [status, deployments] of Object.entries(deploymentsByStatus)) {
            ...
            venusLogger.info("Processing status directory", { statusDir, status, deploymentCount: deployments.length });

            // Ensure status directory exists
            if (!fs.existsSync(statusDir)) {
                venusLogger.info("Creating status directory", { statusDir });
            ...
            await Promise.all(deployments.map(async (deployment) => {
                ...
                venusLogger.debug("Saving deployment file", { filePath, dseq: deployment.dseq });
        ...
        venusLogger.error("Failed to fetch deployments", {
            error: error instanceof Error ? error.message : String(error),
...
export async function saveDeploymentInfo(deploymentInfo: DeploymentInfo, filePath: string): Promise<void> {
    venusLogger.info("Saving deployment info", {
    ...
    try {
        ...
        venusLogger.debug("Deployment info saved successfully");
    } catch (error) {
        venusLogger.error("Failed to save deployment info", {
            error: error instanceof Error ? error.message : String(error),
            stack: error instanceof Error ? error.stack : undefined,
            filePath
        });
...
export async function loadDeploymentInfo(filePath: string): Promise<DeploymentInfo> {
    venusLogger.info("Loading deployment info", { filePath });
    ...
    try {
        ...
        venusLogger.debug("Deployment info loaded successfully", {
            dseq: deploymentInfo.dseq,
            owner: deploymentInfo.owner
        });
        ...
        venusLogger.error("Failed to load deployment info", {
            error: error instanceof Error ? error.message : String(error),
            stack: error instanceof Error ? error.stack : undefined,
            filePath
        });
...
export const getDeploymentApiAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        venusLogger.debug("Validating get deployments request", { message });
        try {
            ...
            venusLogger.error("Get deployments validation failed", {
    ...
    handler: async (
        ...
        venusLogger.info("Starting deployment API request", { actionId });
        ...
        try {
            ...
            if (callback) {
                venusLogger.info("=== Preparing callback response for deployments ===", {
                    hasCallback: true,
                    actionId,
            ...
            venusLogger.error("Get deployments request failed", {
                error: error instanceof Error ? error.message : String(error),
                actionId
            });
            ...
            if (callback) {
                ...
                venusLogger.info("=== Error callback executed ===", {
                    actionId,
                    timestamp: new Date().toISOString()
                });
            }
