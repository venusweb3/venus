...
async function initializeClient(runtime: IAgentRuntime) {
    venusLogger.info("=== Initializing Client for Deployment Closure ===");
    ...
    venusLogger.info("Client initialization complete", {
...
async function verifyDeploymentStatus(runtime: IAgentRuntime, dseq: string): Promise<boolean> {
    venusLogger.info("Verifying deployment status", { dseq });
...
async function closeSingleDeployment(
    ...
    venusLogger.info("Closing single deployment", { dseq });
    ...
    try {
        ...
        // Send transaction
        venusLogger.info("Sending close deployment transaction", { dseq });
        ...
        venusLogger.info("Deployment closed successfully", {
            dseq,
        ...
        venusLogger.error("Failed to close deployment", {
            dseq,
...
async function closeAllDeployments(
    ...
    venusLogger.info("Closing all active deployments");
    ...
    try {
        ...
        if (activeDeployments.length === 0) {
            venusLogger.info("No active deployments found to close");
            return { success: [], failed: [] };
        }

        venusLogger.info("Found active deployments to close", {
        ...
        venusLogger.info("Finished closing deployments", results);
        return results;
    } catch (error) {
        venusLogger.error("Failed to close deployments", {
            error: error instanceof Error ? error.message : String(error),
            code: error instanceof AkashError ? error.code : AkashErrorCode.DEPLOYMENT_CLOSE_FAILED,
            stack: error instanceof Error ? error.stack : undefined
        });
...
export const closeDeploymentAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        venusLogger.debug("=== Starting Close Deployment Validation ===");
        venusLogger.debug("Validating close deployment request", { message });
        ...
        try {
            ...
            const config = await validateAkashConfig(runtime);
            venusLogger.debug("Validating parameters", { params });
            ...
            venusLogger.error("Close deployment validation failed", {
    ...
    handler: async (
        ...
        venusLogger.info("=== Starting Close Deployment Request ===", {
            actionId,
        ...
        try {
            ...
            if (params.closeAll) {
                ...
                if (callback) {
                    venusLogger.info("=== Preparing callback response for bulk closure ===", {
                        hasCallback: true,
                        actionId,
                ...
                if (callback) {
                    venusLogger.info("=== Preparing callback response for single closure ===", {
                        hasCallback: true,
                        actionId,
                        dseq: params.dseq,
                        success
            ...
            venusLogger.error("Close deployment request failed", {
                error: error instanceof Error ? error.message : String(error),
                code: error instanceof AkashError ? error.code : AkashErrorCode.DEPLOYMENT_CLOSE_FAILED,
                actionId
            });
