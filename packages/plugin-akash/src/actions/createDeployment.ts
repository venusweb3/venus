import { Action, venusLogger } from "@venusos/core";
import { IAgentRuntime, Memory, State, HandlerCallback, Content, ActionExample } from "@venusos/core";
...
function saveCertificate(certificate: CertificatePem) {
    venusLogger.debug("Saving certificate to file", { path: CERTIFICATE_PATH });
    try {
        ...
        venusLogger.debug("Certificate saved successfully");
    } catch (error) {
        venusLogger.error("Failed to save certificate", {
...
function loadCertificate(path: string): CertificatePem {
    venusLogger.debug("Loading certificate from file", { path });
    try {
        const json = fs.readFileSync(path, "utf8");
        const certificate = JSON.parse(json);
        venusLogger.debug("Certificate loaded successfully", {
        ...
        venusLogger.error("Failed to load certificate", {
            error: error instanceof Error ? error.message : String(error),
            path
        });
        throw error;
...
const DEFAULT_SDL_PATH = (() => {
    ...
    // venusLogger.info("=== Starting SDL Path Resolution in createDeployment ===", {
    //     currentFileUrl,
    //     cwd: process.cwd(),
    ...
    if (!fs.existsSync(sdlPath)) {
        venusLogger.warn("Default SDL path not found", {
            sdlPath,
            exists: false
        });
    }
...
const loadSDLFromFile = (filePath: string): string => {
    // venusLogger.info("=== Loading SDL File ===", {
    ...
    try {
        ...
        if (!filePath.includes('plugin-akash') && filePath !== DEFAULT_SDL_PATH) {
            ...
            // venusLogger.info("Adjusting SDL path", {
        ...
        // venusLogger.info("Attempting to load SDL from possible paths", {
        //     possiblePaths,
        ...
        for (const tryPath of possiblePaths) {
            if (fs.existsSync(tryPath)) {
                ...
                venusLogger.info("SDL file loaded successfully from", {
                    path: tryPath
                });
                return content;
            }
        ...
        venusLogger.error("Failed to read SDL file", {
            filePath,
            error: error instanceof Error ? error.message : String(error)
        });
...
async function initializeWallet(mnemonic: string) {
    venusLogger.debug("=== Initializing Wallet ===", {
    ...
    if (words.length !== 12 && words.length !== 24) {
        ...
        venusLogger.error("Mnemonic validation failed", {
            error,
    ...
    try {
        venusLogger.debug("Creating wallet with mnemonic", {
            wordCount: words.length,
            mnemonicPreview: words.slice(0, 3).join(' ') + '...'
        });
        ...
        venusLogger.debug("Wallet initialized successfully", {
        ...
        venusLogger.error("Wallet initialization failed", {
            error: error instanceof Error ? error.message : String(error),
...
async function setupClient(wallet: DirectSecp256k1HdWallet, rpcEndpoint: string) {
    ...
    venusLogger.info("=== Setting up Stargate Client ===", {
    ...
    for (const endpoint of rpcEndpoints) {
        try {
            venusLogger.debug("Attempting to connect to RPC endpoint", {
                endpoint,
            ...
            venusLogger.debug("Registry created for endpoint", {
                endpoint,
            ...
            try {
                ...
                venusLogger.info("Stargate client setup successful", {
                    endpoint,
                    height,
                ...
                venusLogger.error("Failed to get chain height", {
                    endpoint,
                    attempt: rpcEndpoints.indexOf(endpoint) + 1,
            ...
            venusLogger.error("Failed to connect to RPC endpoint", {
                endpoint,
                attempt: rpcEndpoints.indexOf(endpoint) + 1,
                error: error instanceof Error ? error.message : String(error),
                stack: error instanceof Error ? error.stack : undefined
    ...
    venusLogger.error("All RPC endpoints failed", {
...
async function fetchBid(dseq: number, owner: string, rpcEndpoint: string) {
    venusLogger.info("=== Starting Bid Fetch Process ===", {
        dseq,
        owner,
    ...
    for (let retry = 0; retry < maxRetries; retry++) {
        try {
            venusLogger.debug("Connecting to RPC for bid fetch", {
                rpcEndpoint,
                attempt: retry + 1,
                maxRetries
            });
            ...
            venusLogger.debug("RPC connection established", {
            ...
            venusLogger.debug("Starting bid polling loop", {
                timeout: "5 minutes",
            ...
            while (Date.now() - startTime < timeout) {
                ...
                venusLogger.debug("Polling for bids", {
                    dseq,
                ...
                try {
                    ...
                    if (bids.bids.length > 0 && bids.bids[0].bid !== undefined) {
                        venusLogger.info("Bid found successfully", {
                            dseq,
                            owner: owner.substring(0, 6),
                    ...
                    venusLogger.warn("Temporary error during bid polling", {
            ...
            venusLogger.error("Bid fetch timeout", {
                dseq,
                owner: owner.substring(0, 6),
                timeout: "5 minutes",
                attempt: retry + 1
            ...
            lastError = error instanceof Error ? error : new Error(String(error));
            venusLogger.error("Error during bid fetch", {
                error: error instanceof Error ? error.message : String(error),
                stack: error instanceof Error ? error.stack : undefined,
                dseq,
                owner: owner.substring(0, 6),
            ...
            if (retry < maxRetries - 1) {
                ...
                venusLogger.info("Retrying bid fetch after delay", {
                    delay,
                    nextAttempt: retry + 2,
                    maxRetries
                });
    ...
    venusLogger.error("All bid fetch attempts failed", {
        dseq,
        owner: owner.substring(0, 6),
        attempts: maxRetries,
...
async function createLease(deployment: any, wallet: DirectSecp256k1HdWallet, client: SigningStargateClient, rpcEndpoint: string): Promise<any> {
    ...
    venusLogger.info("Starting lease creation", { dseq, owner });

    try {
        venusLogger.debug("Fetching bid for lease creation");
        const bid = await fetchBid(dseq, owner, rpcEndpoint);
        const accounts = await wallet.getAccounts();

        if (bid.bidId === undefined) {
            venusLogger.error("Invalid bid - missing bidId", { dseq, owner });
            throw new AkashError("Bid ID is undefined", AkashErrorCode.INVALID_BID);
        }

        venusLogger.debug("Creating lease message", {
            dseq,
            owner,
            bidId: bid.bidId
        });
        ...
        venusLogger.info("Broadcasting lease creation transaction");
        const tx = await client.signAndBroadcast(accounts[0].address, [msg], fee, "create lease");

        if (tx.code !== 0) {
            venusLogger.error("Lease creation failed", {
                dseq,
                owner,
                code: tx.code,
                rawLog: tx.rawLog
        ...
        venusLogger.info("Lease created successfully", {
            dseq,
            owner,
        ...
        venusLogger.error("Error during lease creation", {
            error,
            dseq,
            owner
        });
...
async function queryLeaseStatus(lease: any, providerUri: string, certificate: CertificatePem): Promise<LeaseStatus | undefined> {
    const id = lease.id;
    venusLogger.info("Querying lease status", {
        dseq: id?.dseq,
        gseq: id?.gseq,
        oseq: id?.oseq,
        providerUri
    ...
    if (id === undefined) {
        venusLogger.error("Invalid lease - missing ID");
    ...
    venusLogger.debug("Setting up request", {
        providerUri,
        leasePath,
        hasCert: !!certificate.cert,
    ...
    while (retryCount < MAX_RETRIES) {
        try {
            ...
            venusLogger.debug("Making request", {
                url: fullUrl,
                method: 'GET',
            ...
            try {
                ...
                if (response.status !== 200) {
                    venusLogger.warn("Non-OK response from lease status query", {
                    ...
                    if (response.status === 404) {
                        venusLogger.debug("Deployment not ready yet (404)", {
                            dseq: id.dseq,
                            retryCount
                        });
                        return undefined;
                ...
                venusLogger.debug("Lease status received", {
                    dseq: id.dseq,
            ...
            venusLogger.warn("Error during lease status query", {
                error: error instanceof Error ? error.message : String(error),
                stack: error instanceof Error ? error.stack : undefined,
                dseq: id.dseq,
                providerUri,
            ...
            if (retryCount < MAX_RETRIES - 1) {
                ...
                venusLogger.debug("Retrying after error", {
                    delay,
            ...
            if (error instanceof Error &&
                ...
                venusLogger.info("Returning undefined after max retries", {
                    dseq: id.dseq,
                    error: error.message
                });
                return undefined;
    ...
    venusLogger.info("Max retries reached, returning undefined", {
        dseq: id.dseq,
        maxRetries: MAX_RETRIES
    });
    return undefined;
...
async function sendManifest(sdl: SDL, lease: any, certificate: CertificatePem, rpcEndpoint: string) {
    venusLogger.info("Starting manifest send process");
    ...
    try {
        ...
        venusLogger.debug("Getting provider info", { provider });
        ...
        if (tx.provider === undefined) {
            venusLogger.error("Provider not found", { provider });
            throw new AkashError(
        ...
        venusLogger.debug("Provider info retrieved", {
            provider,
        ...
        venusLogger.info("Sending manifest to provider", {
            dseq,
            provider,
        ...
        try {
            ...
            venusLogger.debug("Making manifest request", {
                url: fullUrl,
                method: 'PUT',
                manifestLength: manifest.length
            });
            ...
            if (response.status !== 200) {
                venusLogger.error("Failed to send manifest", {
                    statusCode: response.status,
                    statusText: response.statusText,
                    dseq
                });
            ...
            venusLogger.info("Manifest sent successfully", { dseq });
        } finally {
            httpsAgent.destroy();
        }

        // Wait for deployment to start
        venusLogger.info("Waiting for deployment to start", { dseq });
        const startTime = Date.now();
        ...
        while (Date.now() - startTime < timeout) {
            const elapsedTime = Math.round((Date.now() - startTime) / 1000);
            venusLogger.debug("Checking deployment status", {
                dseq,
            ...
            try {
                ...
                if (status === undefined) {
                    consecutiveErrors++;
                    venusLogger.debug("Status check returned undefined", {
                        dseq,
                        consecutiveErrors,
                    ...
                    if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
                        venusLogger.warn("Too many consecutive undefined status responses", {
                            dseq,
                            consecutiveErrors
                        });
                ...
                for (const [name, service] of Object.entries<{ uris?: string[] }>(status.services)) {
                    if (service.uris) {
                        ...
                        venusLogger.info("Service is available", {
                            name,
                            rawUrl,
                            serviceUrl,
                            dseq
                ...
                consecutiveErrors++;
                const errorMessage = error instanceof Error ? error.message : String(error);
                venusLogger.warn("Error checking deployment status", {
                    error: errorMessage,
                    dseq,
                    consecutiveErrors,
                    maxConsecutiveErrors: MAX_CONSECUTIVE_ERRORS
                ...
                if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
                    venusLogger.error("Too many consecutive errors checking deployment status", {
                        dseq,
                        consecutiveErrors,
                        error: errorMessage
                    });
        ...
        venusLogger.error("Deployment start timeout", {
            dseq,
            timeout: "10 minutes"
        });
        throw new AkashError(
        ...
        venusLogger.error("Error during manifest send process", {
            error: error instanceof Error ? error.message : String(error),
            stack: error instanceof Error ? error.stack : undefined,
            dseq: lease.id.dseq
        });
...
async function loadOrCreateCertificate(wallet: DirectSecp256k1HdWallet, client: SigningStargateClient): Promise<CertificatePem> {
    venusLogger.info("=== Starting Certificate Creation/Loading Process ===");
    try {
        const accounts = await wallet.getAccounts();
        const address = accounts[0].address;
        venusLogger.debug("Got wallet address for certificate", {
            address,
        ...
        if (fs.existsSync(CERTIFICATE_PATH)) {
            venusLogger.info("Found existing certificate file", { path: CERTIFICATE_PATH });
            const cert = loadCertificate(CERTIFICATE_PATH);
            venusLogger.debug("Loaded existing certificate", {
                hasCert: !!cert.cert,
        ...
        venusLogger.info("No existing certificate found, creating new one", { address });
        const certificate = certificateManager.generatePEM(address);
        venusLogger.debug("Certificate generated", {
            hasCert: !!certificate.cert,
            hasPrivateKey: !!certificate.privateKey,
            hasPublicKey: !!certificate.publicKey,
        ...
        venusLogger.info("Broadcasting certificate to network", {
            address,
            certLength: certificate.cert?.length,
            publicKeyLength: certificate.publicKey?.length
        });
        ...
        const result = await cert.broadcastCertificate(
            ...
            venusLogger.error("Certificate broadcast failed", {
                error: error instanceof Error ? error.message : String(error),
                stack: error instanceof Error ? error.stack : undefined,
                address,
                certLength: certificate.cert?.length
        ...
        if (result.code !== 0) {
            ...
            venusLogger.error("Certificate broadcast returned error code", {
                code: result.code,
        ...
        venusLogger.info("Certificate broadcast successful", {
            code: result.code,
            txHash: result.transactionHash,
        ...
        venusLogger.info("Certificate saved to file", { path: CERTIFICATE_PATH });

        venusLogger.info("Certificate process completed successfully", {
            hasCert: !!certificate.cert,
            hasPrivateKey: !!certificate.privateKey,
            hasPublicKey: !!certificate.publicKey,
            path: CERTIFICATE_PATH
        ...
        venusLogger.error("Certificate creation/broadcast process failed", {
            error: error instanceof Error ? error.message : String(error),
            stack: error instanceof Error ? error.stack : undefined,
            path: CERTIFICATE_PATH
        });
...
async function parseSDL(sdlContent: string): Promise<SDL> {
    try {
        ...
        venusLogger.info("Starting SDL parsing process", {
        ...
        venusLogger.debug("Initial SDL parsing successful", {
        ...
        venusLogger.info("SDL validation completed", {
        ...
        venusLogger.error("Failed to parse SDL", {
            error: error instanceof Error ? error.message : String(error),
            stack: error instanceof Error ? error.stack : undefined,
...
export const createDeploymentAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        venusLogger.debug("=== Starting Deployment Validation ===");
        venusLogger.debug("Validating deployment request", { message });

        // Check if plugin is properly loaded
        if (!isPluginLoaded(runtime, "akash")) {
            venusLogger.error("Akash plugin not properly loaded during validation");
            return false;
        }

        try {
            const params = message.content as Partial<CreateDeploymentContent>;
            venusLogger.debug("Checking SDL content", { params });
            ...
            venusLogger.debug("Validating SDL format");
            try {
                // Clean up SDL content by taking only the part after the YAML document separator
                const yamlSeparatorIndex = sdlContent.indexOf('---');
                if (yamlSeparatorIndex === -1) {
                ...
                venusLogger.debug("SDL format validation successful", {
                    groups: sdl.groups(),
                    groupCount: sdl.groups().length
                });
            } catch (sdlError) {
                venusLogger.error("SDL format validation failed", { error: sdlError });
                throw new AkashError(
            ...
            venusLogger.debug("Validation completed successfully");
            return true;
        } catch (error) {
            venusLogger.error("Deployment validation failed", {
    ...
    handler: async (
        ...
        venusLogger.info("=== Starting Deployment Creation ===", {
            actionId,
        ...
        try {
            venusLogger.debug("=== Validating Akash Configuration ===");
            const config = await validateAkashConfig(runtime);
            venusLogger.debug("Configuration validated successfully", {
            ...
            venusLogger.debug("=== Processing Deployment Parameters ===", {
                hasSDL: !!params.sdl,
            ...
            venusLogger.debug("SDL content loaded", {
                source: sdlSource,
                contentLength: sdlContent.length
            });

            if (params.deposit && !validateDeposit(params.deposit)) {
                venusLogger.error("Invalid deposit format", {
            ...
            venusLogger.info("=== Initializing Wallet and Client ===");
            const wallet = await initializeWallet(config.AKASH_MNEMONIC);
            const accounts = await wallet.getAccounts();
            const address = accounts[0].address;
            venusLogger.debug("Wallet initialized", {
                address,
                accountCount: accounts.length
            });

            // Setup client
            venusLogger.debug("Setting up Stargate client");
            const client = await setupClient(wallet, config.RPC_ENDPOINT);
            venusLogger.debug("Client setup completed", {
                rpcEndpoint: config.RPC_ENDPOINT
            });

            // Load or create certificate
            venusLogger.info("=== Setting up Certificate ===");
            const certificate = await loadOrCreateCertificate(wallet, client);
            venusLogger.debug("Certificate setup completed", {
                hasCert: !!certificate.cert,
                hasPrivateKey: !!certificate.privateKey,
                hasPublicKey: !!certificate.publicKey
            });
            ...
            // Parse SDL
            venusLogger.info("=== Parsing SDL Configuration ===");
            let sdl: SDL;
            try {
                sdl = await parseSDL(sdlContent);
                venusLogger.debug("SDL parsed successfully", {
                    groupCount: sdl.groups().length,
                    groups: sdl.groups(),
                    version: await sdl.manifestVersion()
                });
            } catch (sdlError) {
                venusLogger.error("SDL parsing failed", {
            ...
            venusLogger.info("=== Creating Deployment Message ===");
            const blockHeight = await client.getHeight();
            venusLogger.debug("Current block height", { blockHeight });
            ...
            venusLogger.debug("Deployment object created", {
            ...
            venusLogger.info("=== Broadcasting Deployment Transaction ===", {
                owner: address,
                dseq: blockHeight,
            ...
            const result = await withRetry(async () => {
                venusLogger.debug("Attempting to sign and broadcast transaction", {
                    attempt: 'current',
                ...
                venusLogger.debug("Transaction broadcast result", {
                    code: txResult.code,
                ...
                if (txResult.code !== 0) {
                    venusLogger.error("Transaction failed", {
                        code: txResult.code,
                        rawLog: txResult.rawLog,
                        groups: deployment.groups
                    });
            ...
            venusLogger.info("=== Deployment Created Successfully ===", {
                txHash: result.transactionHash,
                owner: address,
                dseq: blockHeight,
                actionId,
            ...
            // Create lease
            venusLogger.debug("=== Creating Lease ===");
            const lease = await createLease(deployment, wallet, client, config.RPC_ENDPOINT);
            venusLogger.debug("Lease created", {
                leaseId: lease.id,
                dseq: deployment.id.dseq
            });

            // Send manifest
            venusLogger.debug("=== Sending Manifest ===");
            const serviceUrl = await sendManifest(sdl, lease, certificate, config.RPC_ENDPOINT);
            venusLogger.debug("Manifest sent successfully", {
                serviceUrl
            });

            if (callback) {
                venusLogger.info("=== Preparing callback response for deployment creation ===", {
                    hasCallback: true,
                    actionId,
                ...
                venusLogger.info("=== Executing callback with response ===", {
                    actionId,
                ...
                venusLogger.info("=== Callback executed successfully ===", {
                    actionId,
                    timestamp: new Date().toISOString()
                });
            }

            venusLogger.info("=== Deployment Process Completed Successfully ===", {
                actionId,
                txHash: result.transactionHash,
                dseq: blockHeight
            });
            ...
            venusLogger.error("=== Deployment Creation Failed ===", {
                error: error instanceof AkashError ? {
                    category: error.category,
                    code: error.code,
                    message: error.message,
            ...
            if (callback) {
                venusLogger.info("=== Preparing error callback response ===", {
                    actionId,
                    hasCallback: true,
                ...
                venusLogger.info("=== Executing error callback ===", {
                    actionId,
                ...
                venusLogger.info("=== Error callback executed successfully ===", {
                    actionId,
                    timestamp: new Date().toISOString()
                });
            }
