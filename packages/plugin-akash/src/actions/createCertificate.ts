...
function loadCertificate(): CertificatePem {
    venusLogger.debug("Loading certificate from file", { path: CERTIFICATE_PATH });
    try {
...
async function setupClient(wallet: DirectSecp256k1HdWallet, rpcEndpoint: string): Promise<SigningStargateClient> {
    ...
    for (const endpoint of rpcEndpoints) {
        try {
            ...
            venusLogger.debug("Client setup completed successfully", {
                endpoint,
            ...
            venusLogger.warn("Failed to connect to RPC endpoint", {
                endpoint,
                error: error instanceof Error ? error.message : String(error),
...
export const createCertificateAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        venusLogger.debug("=== Starting Certificate Validation ===");
        try {
            ...
            venusLogger.debug("Certificate validation completed successfully");
            return true;
        } catch (error) {
            venusLogger.error("Certificate validation failed", {
    ...
    handler: async (
        ...
        venusLogger.info("=== Starting Certificate Creation/Loading ===", { actionId });
        ...
        try {
            ...
            try {
                ...
                if (fs.existsSync(CERTIFICATE_PATH) && !params.overwrite) {
                    venusLogger.info("Loading existing certificate");
                ...
                // Initialize wallet
                venusLogger.info("Initializing wallet for certificate creation");
                ...
                venusLogger.info("Generating new certificate");
                ...
                venusLogger.error("Failed to create/load certificate", {
                    error: error instanceof Error ? error.message : String(error),
                    stack: error instanceof Error ? error.stack : undefined
                });
            ...
            venusLogger.error("Certificate operation failed", {
                error: error instanceof Error ? error.message : String(error),
