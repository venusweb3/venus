...
// venusLogger.info("Default SDL path initialized", { DEFAULT_SDL_PATH });
// venusLogger.info("Loading SDL from file", { filePath });
// venusLogger.info("Resolved SDL file path", { resolvedPath });
// venusLogger.error("SDL file not found", { resolvedPath });
// venusLogger.info("SDL file loaded successfully", { content });
// venusLogger.error("Failed to read SDL file", { error });
// venusLogger.error("SDL validation failed", { error });
// venusLogger.info("Using provided SDL content");
// venusLogger.info("Loading SDL from file", { path: params.sdlFile });
// venusLogger.info("Loading default SDL", { path: DEFAULT_SDL_PATH });
// venusLogger.debug("Parsing SDL content and generating manifest");
...
const validateSDL = (sdlContent: string, validationLevel: string = "strict"): boolean => {
    try {
        ...
        if (validationLevel === "none") {
            // venusLogger.debug("Skipping SDL validation (validation level: none)");
            return true;
        }
        ...
        // venusLogger.debug("SDL validation successful", {
        //     validationLevel,
        //     checkedSections: sectionsToCheck
        // });
        return true;
    } catch (error) {
        venusLogger.error("SDL validation failed", {
            error: error instanceof Error ? error.message : String(error),
            validationLevel
        });
        return false;
...
export const getManifestAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        venusLogger.debug("Validating manifest request", { message });
        try {
            ...
            venusLogger.error("Manifest validation failed", {
    ...
    handler: async (
        ...
        venusLogger.info("Starting manifest operation", { actionId });
        ...
        try {
            ...
            try {
                ...
                if (params.sdl) {
                    ...
                    venusLogger.info("Loaded SDL from file", { path: params.sdlFile });
                } else {
                    const sdlPath = config.AKASH_SDL || DEFAULT_SDL_PATH;
                    sdlContent = loadSDLFromFile(sdlPath);
                    venusLogger.info("Using SDL from environment", { path: sdlPath });
                }
                ...
                if (manifestMode === "validate_only") {
                    venusLogger.info("Validation successful (validate_only mode)");
                    if (callback) {
                ...
                if (config.AKASH_MANIFEST_PATH) {
                    ...
                    venusLogger.info("Manifest saved", { path: manifestPath });
                }
                ...
                venusLogger.error("Manifest operation failed", {
