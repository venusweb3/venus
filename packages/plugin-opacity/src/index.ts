...
export class OpacityAdapter implements IVerifiableInferenceAdapter {
    ...
    async generateText(
        ...
        venusLogger.log("Generating text with options:", {
        ...
        try {
            ...
            venusLogger.debug("Request body:", JSON.stringify(body, null, 2));
            ...
            venusLogger.debug("Making request to Cloudflare with:", {
                endpoint,
                headers: {
                    ...requestHeaders,
            ...
            try {
                ...
                venusLogger.error("Invalid JSON body:", body);
                throw new Error("Failed to create valid JSON request body");
            }
            venusLogger.debug("Request body:", requestBody);
            ...
            if (!cloudflareResponse.ok) {
                ...
                venusLogger.error("Cloudflare error response:", {
            ...
            venusLogger.debug("Cloudflare response:", {
                status: cloudflareResponse.status,
                statusText: cloudflareResponse.statusText,
    ...
    async generateProof(baseUrl: string, logId: string) {
        ...
        venusLogger.debug("Fetching proof for log ID:", logId);
        if (!response.ok) {
