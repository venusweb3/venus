...
export class LensInteractionManager {
    ...
    private async handleInteractions() {
        venusLogger.info("Handle Lens interactions");
    ...
    private async handlePublication({
        ...
        if (publication.by.id === agent.id) {
            venusLogger.info("skipping cast from bot itself", publication.id);
            return;
        }

        if (!memory.content.text) {
            venusLogger.info("skipping cast with no text", publication.id);
