...
export class LensPostManager {
    ...
    private async generateNewPublication() {
        venusLogger.info("Generating new publication");
        try {
            ...
            if (this.runtime.getSetting("LENS_DRY_RUN") === "true") {
                venusLogger.info(`Dry run: would have posted: ${content}`);
                return;
            }
            ...
            try {
                ...
                venusLogger.info(`[Lens Client] Published ${publication.id}`);
                ...
                venusLogger.error("Error sending publication:", error);
            }
        } catch (error) {
            venusLogger.error("Error generating new publication:", error);
        }
    }
}
