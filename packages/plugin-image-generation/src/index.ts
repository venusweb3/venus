import { venusLogger, generateText } from "@venusos/core";
import {
    Action,
    HandlerCallback,
    IAgentRuntime,
...
const imageGeneration: Action = {
    ...
    handler: async (
        ...
        venusLogger.log("Composing state for message:", message);
        state = (await runtime.composeState(message)) as State;
        const userId = runtime.agentId;
        venusLogger.log("User ID:", userId);
...
Ensure that your prompt is detailed, vivid, and incorporates all the elements mentioned above while staying true to the content and the specified style. LIMIT the image prompt 50 words or less. \n\nWrite a prompt. Only include the prompt and nothing else.`;
        ...
        venusLogger.log("Image prompt received:", imagePrompt);
        const imageSettings = runtime.character?.settings?.imageSettings || {};
        venusLogger.log("Image settings:", imageSettings);

        const res: { image: string; caption: string }[] = [];

        venusLogger.log("Generating image with prompt:", imagePrompt);
        ...
        if (images.success && images.data && images.data.length > 0) {
            ...
            for (let i = 0; i < images.data.length; i++) {
                ...
                venusLogger.log(`Processing image ${i + 1}:`, filename);
                ...
                try {
                    ...
                    venusLogger.error("Caption generation failed, using default caption:", error);
                }*/
            ...
            venusLogger.error("Image generation failed or returned no data.");
        }
    },
    examples: [
