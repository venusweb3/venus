...
const generateVideo = async (prompt: string, runtime: IAgentRuntime) => {
    ...
    try {
        venusLogger.log("Starting video generation with prompt:", prompt);
        ...
        if (!response.ok) {
            ...
            venusLogger.error("Luma API error:", {
        ...
        while (status !== "completed" && status !== "failed") {
            ...
            if (!statusResponse.ok) {
                ...
                venusLogger.error("Status check error:", {
            ...
            venusLogger.log("Status check response:", statusData);
        ...
        venusLogger.error("Video generation error:", error);
        return {
            success: false,
...
const videoGeneration: Action = {
    ...
    validate: async (runtime: IAgentRuntime, _message: Memory) => {
        venusLogger.log("Validating video generation action");
        const lumaApiKey = runtime.getSetting("LUMA_API_KEY");
        venusLogger.log("LUMA_API_KEY present:", !!lumaApiKey);
        return !!lumaApiKey;
    },
    handler: async (
        ...
        venusLogger.log("Video generation request:", message);
        ...
        venusLogger.log("Video prompt:", videoPrompt);
        ...
        try {
            ...
            venusLogger.error(`Failed to generate video. Error: ${error}`);
            callback({
