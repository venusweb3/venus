import { AwsS3Service } from "@venusos/plugin-node";
import {
    composeContext,
    venusLogger,
    generateImage,
...
import {
    saveBase64Image,
    saveHeuristImage,
} from "@venusos/plugin-image-generation";
...
export async function createNFTMetadata({
    ...
    venusLogger.log("NFT Prompt:", nftPrompt);
    if (images.success && images.data && images.data.length > 0) {
        const image = images.data[0];
        const filename = `${tokenId}`;
        if (image.startsWith("http")) {
            venusLogger.log("Generating image url:", image);
        }
...
export async function createNFT({
    ...
    if (nftInfo) {
        ...
        venusLogger.log("NFT ID:", nftAddressRes.address);
        return {
            network: "solana",
