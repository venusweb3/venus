...
const generateTTS = async (prompt: string, voice: string, runtime: IAgentRuntime) => {
    ...
    try {
        venusLogger.log("Starting TTS generation with prompt:", prompt);
        ...
        venusLogger.error("TTS generation error:", error);
        return {
            success: false,
...
const TTSGeneration: Action = {
    ...
    validate: async (runtime: IAgentRuntime, _message: Memory) => {
        venusLogger.log("Validating TTS action");
        const FalApiKey = runtime.getSetting("FAL_API_KEY");
        venusLogger.log("FAL_API_KEY present:", !!FalApiKey);
        return !!FalApiKey;
    },
    handler: async (
        ...
        venusLogger.log("TTS request:", message);
        ...
        venusLogger.log("TTS prompt:", TTSPrompt);
        ...
        venusLogger.log("Starting TTS generation with prompt:", prompt, "and voice:", target_voice);
        ...
        try {
            ...
            if (result.success && result.data.audio.url) {
                ...
                venusLogger.log("Audio Duration:", result.data.audio.duration);
                callback(
                    {
            ...
            venusLogger.error(`Failed to generate TTS. Error: ${error}`);
            callback({
