...
export class DiscordClient extends EventEmitter {
    ...
    async stop() {
        try {
            ...
            venusLogger.error("client-discord instance stop err", e);
        }
    }

    private async onClientReady(readyClient: { user: { tag: any; id: any } }) {
        venusLogger.success(`Logged in as ${readyClient.user?.tag}`);
        ...
        try {
            ...
            venusLogger.success("Slash commands registered");
        } catch (error) {
        ...
        venusLogger.success("Use this URL to add the bot to your server:");
        venusLogger.success(
    ...
    async handleReactionAdd(reaction: MessageReaction, user: User) {
        try {
            venusLogger.log("Reaction added");

            // Early returns
            if (!reaction || !user) {
                venusLogger.warn("Invalid reaction or user");
                return;
            }
            ...
            if (!userIdUUID || !roomId) {
                venusLogger.error("Invalid user ID or room ID", {
                    userIdUUID,
                    roomId,
                });
                return;
            ...
            try {
                ...
                venusLogger.debug("Reaction memory created", {
                ...
                if (error.code === "23505") {
                    // Duplicate key error
                    venusLogger.warn("Duplicate reaction memory, skipping", {
                        reactionId: reactionUUID,
                    });
                    return;
                }
            ...
            venusLogger.error("Error handling reaction:", error);
        }
    }

    async handleReactionRemove(reaction: MessageReaction, user: User) {
        venusLogger.log("Reaction removed");
        // if (user.bot) return;
...
export const DiscordClientInterface: venusClient = {
    start: async (runtime: IAgentRuntime) => new DiscordClient(runtime),
    stop: async (runtime: IAgentRuntime) => {
        try {
            // stop it
            venusLogger.log("Stopping discord client", runtime.agentId);
            await runtime.clients.discord.stop();
        } catch (e) {
            venusLogger.error("client-discord interface stop error", e);
        }
    },
};
