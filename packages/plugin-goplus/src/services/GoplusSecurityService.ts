import { IAgentRuntime, ModelClass, Service, ServiceType, venusLogger, generateObjectDeprecated, generateText } from "@venusos/core";
...
export class GoplusSecurityService extends Service implements IGoplusSecurityService {
    ...
    async check(text: string): Promise<string> {
        try {
            venusLogger.log("check input text", text);
            ...
            venusLogger.log("check generateObjectDeprecated text", obj);
