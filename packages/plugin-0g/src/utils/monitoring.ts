...
export const logSecurityEvent = (
    ...
    venusLogger.info('Security event', securityEvent);

    // For high severity events, also log as error
    if (severity === 'high') {
        venusLogger.error('High severity security event', securityEvent);
    }
};
...
export const monitorUpload = (metrics: Omit<UploadMetrics, 'timestamp'>): void => {
    ...
    venusLogger.info('Upload metrics', uploadMetrics);

    // Log errors if present
    if (!metrics.success && metrics.error) {
        venusLogger.error('Upload failed', {
