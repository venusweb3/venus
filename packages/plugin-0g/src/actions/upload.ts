...
import { composeContext } from "@venusos/core";
...
function isUploadContent(
    ...
    venusLogger.debug("Validating upload content", { content });
...
export const zgUpload: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        venusLogger.debug("Starting ZG_UPLOAD validation", { messageId: message.id });
        ...
        try {
            ...
            venusLogger.debug("Checking ZeroG settings", {
            ...
            if (!hasRequiredSettings) {
                ...
                venusLogger.error("Missing required ZeroG settings", {
                    missingSettings,
            ...
            if (isNaN(config.maxFileSize) || config.maxFileSize <= 0) {
                venusLogger.error("Invalid ZEROG_MAX_FILE_SIZE setting", {
            ...
            if (!config.allowedExtensions || config.allowedExtensions.length === 0) {
                venusLogger.error("Invalid ZEROG_ALLOWED_EXTENSIONS setting", {
            ...
            venusLogger.info("ZG_UPLOAD action settings validated", {
                config,
                messageId: message.id
            });
            return true;
        } catch (error) {
            venusLogger.error("Error validating ZG_UPLOAD settings", {
    ...
    handler: async (
        ...
        venusLogger.info("ZG_UPLOAD action started", {
            messageId: message.id,
        ...
        try {
            ...
            if (!state) {
                venusLogger.debug("No state provided, composing new state");
                state = (await runtime.composeState(message)) as State;
            } else {
                venusLogger.debug("Updating existing state");
                state = await runtime.updateRecentMessageState(state);
            }

            // Compose upload context
            venusLogger.debug("Composing upload context");
            ...
            venusLogger.debug("Generating upload content");
            ...
            venusLogger.debug("Extracted file path", { filePath, content });

            if (!filePath) {
                const error = "File path is required";
                venusLogger.error(error, { messageId: message.id });
                if (callback) {
                    callback({
            ...
            try {
                venusLogger.debug("Initializing security validator", {
            ...
            // Validate file type
            venusLogger.debug("Starting file type validation", { filePath });
            ...
            // Validate file size
            venusLogger.debug("Starting file size validation", { filePath });
            ...
            // Validate file path
            venusLogger.debug("Starting file path validation", { filePath });
            ...
            try {
                ...
                venusLogger.debug("File path sanitized", {
            ...
            try {
                ...
                venusLogger.debug("File stats retrieved", {
                    size: fileStats.size,
                    path: sanitizedPath,
            ...
            try {
                ...
                venusLogger.debug("Initializing ZeroG file", {
                    sanitizedPath,
                    messageId: message.id
                });
                ...
                venusLogger.debug("Generating Merkle tree");
                ...
                venusLogger.info("File root hash generated", {
                    rootHash: merkleTree.rootHash(),
                    messageId: message.id
                });

                // Initialize blockchain connection
                venusLogger.debug("Initializing blockchain connection");
                ...
                venusLogger.info("Starting file upload to ZeroG", {
                ...
                venusLogger.info("File uploaded successfully", {
                ...
                if (cleanupRequired && file) {
                    try {
                        venusLogger.debug("Starting file cleanup", {
                            filePath: sanitizedPath,
                            messageId: message.id
                        });
                        await file.close();
                        ...
                        venusLogger.debug("File cleanup completed successfully", {
                            filePath: sanitizedPath,
                            messageId: message.id
                        });
                    } catch (cleanupError) {
                        monitorCleanup(sanitizedPath, false, cleanupError.message);
                        venusLogger.warn("Failed to cleanup file", {
            ...
            venusLogger.error("Unexpected error during file upload", {
                error: errorMessage,
                stack: error instanceof Error ? error.stack : undefined,
                messageId: message.id
            });
