import { venusLogger, IAgentRuntime, settings, State } from "@venusos/core";
...
export async function fetchWithRetry(
    ...
    for (let i = 0; i < PROVIDER_CONFIG.MAX_RETRIES; i++) {
        try {
            venusLogger.log(`Attempt ${i + 1} for ${url} with chain ${chain}`);
            ...
            venusLogger.error(`Attempt ${i + 1} failed:`, {
    ...
    try {
        ...
        if (state[historyKey]) {
            try {
                ...
                venusLogger.warn("Failed to parse history, resetting", e);
            }
        }
        ...
        venusLogger.error("Failed to manage analyzed tokens history:", {
