import type { Plugin, IAgentRuntime, Memory, State } from "@venusos/core";
import { venusLogger, settings } from "@venusos/core";
import { z } from "zod";
...
function validateSolanaAddress(address: string | undefined): boolean {
    ...
    try {
        ...
        if (!/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(address)) {
            venusLogger.warn(`Solana address failed format check: ${address}`);
            return false;
        }
        ...
        venusLogger.error(`Address validation error for ${address}:`, error);
        return false;
    }
}
...
export function loadTokenAddresses(): string[] {
    try {
        ...
        venusLogger.log("Loaded token addresses:", {
...
async function updateSellDetails(
    ...
    venusLogger.log("Attempting to update trade performance with data:", {
        sellDetails,
        whereClause: {
            tokenAddress,
            recommenderId,
    ...
    try {
        try {
            ...
            if (!success) {
                venusLogger.warn("Trade update returned false", {
                    tokenAddress,
                    recommenderId,
                    buyTimeStamp: trade.buy_timeStamp,
                });
            ...
            venusLogger.log("Trade performance update completed", {
                success,
                tokenAddress,
                recommenderId,
            ...
            venusLogger.error("Database error during trade update:", {
                error: dbError,
                query: {
                    sellDetails,
                    whereClause: {
        ...
        venusLogger.error("Failed to update trade performance:", {
            error,
            parameters: {
                sellDetails,
                whereClause: {
...
declare module "@venusos/plugin-trustdb" {
...
async function createRabbiTraderPlugin(
    ...
    const resumeTrading = async () => {
        ...
        venusLogger.log(`Analyzing ${tokenAddresses.length} Solana tokens...`);
    ...
    venusLogger.log("Starting GOAT plugin initialization");
    ...
    if (missingSettings.length > 0) {
        ...
        venusLogger.error(errorMsg);
        throw new Error(errorMsg);
    }

    venusLogger.log("Initializing Solana connection...");
    ...
    try {
        ...
        if (twitterConfig.enabled && runtime) {
            venusLogger.log("Starting Twitter client initialization...");
            ...
            venusLogger.log("Twitter service initialized successfully", {
                username: twitterConfig.username,
                dryRun: twitterConfig.dryRun,
            });
        }
    } catch (error) {
        venusLogger.error("Failed to initialize Twitter service:", error);
    }

    venusLogger.log("Initializing Solana plugin components...");
    ...
    try {
        ...
        venusLogger.log("Starting autonomous trading system...");
        ...
        venusLogger.log("Initializing trading loop...");
        await resumeTrading();
        setInterval(resumeTrading, interval);

        venusLogger.log("GOAT plugin initialization completed successfully");
        return plugin;
    } catch (error) {
        venusLogger.error("Failed to initialize plugin components:", error);
        throw new Error(
...
async function analyzeToken(
    ...
    try {
        ...
        venusLogger.log(`Starting analysis for token: ${tokenAddress}`);

        await new Promise((resolve) => setTimeout(resolve, 2000));

        if (!validateSolanaAddress(tokenAddress)) {
            venusLogger.error(`Invalid token address format: ${tokenAddress}`);
            return;
        }
        ...
        venusLogger.log(`Fetching token data for ${tokenAddress}`);
        const tokenData = await tokenProvider.getProcessedTokenData();
        venusLogger.log(`Token data fetched for ${tokenAddress}:`, tokenData);
        ...
        if (!walletPublicKey) {
            venusLogger.error("No wallet public key configured");
            return;
        }
        ...
        venusLogger.log(`Latest trade for ${tokenAddress}:`, latestTrade);
        ...
        venusLogger.error(`Error processing token ${tokenAddress}:`, {
            error: tokenError,
...
async function buy({
    ...
    venusLogger.log(`Trade recommended for ${tokenAddress}:`, result);
        ...
        try {
            ...
            if (tradeResult.success) {
                ...
                if (twitterService && result.recommendedAction === "BUY") {
                    ...
                    venusLogger.log("Skipping tweet due to rate limit");
                }
                ...
                try {
                    ...
                    venusLogger.log(`Created/retrieved recommender:`, {
                        recommender,
                    ...
                    venusLogger.log(`Prepared trade data:`, tradeData);
                    ...
                    venusLogger.error("Failed to record trade performance:", {
                        error,
                        tokenAddress,
                        errorMessage:
                            error instanceof Error
...
async function sell({
    ...
    if (tradeResult.success) {
        venusLogger.log(`Sell executed successfully for ${tokenAddress}:`, {
            signature: tradeResult.signature,
            amount: tradeAmount,
        });
