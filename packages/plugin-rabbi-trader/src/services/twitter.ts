...
export function canTweet(tweetType: "trade" | "market_search" | "shabbat" | "holiday"): boolean {
  ...
  if (currentCount >= MAX_TWEETS_PER_HOUR[tweetType]) {
    venusLogger.warn(`Tweet rate limit reached for ${tweetType}`);
    return false;
  }
...
export class TwitterService {
  ...
  async postTradeAlert(alert: TradeBuyAlert): Promise<boolean> {
    try {
      ...
      if (!canTweet("trade")) {
        venusLogger.warn("Trade tweet rate limit reached");
        return false;
      }

      // Use the correct client structure
      await this.client.post.client.twitterClient.sendTweet(tweetContent);
      venusLogger.log("Successfully posted trade alert to Twitter:", {
        content: tweetContent,
      });

      return true;
    } catch (error) {
      venusLogger.error("Failed to post trade alert to Twitter:", {
