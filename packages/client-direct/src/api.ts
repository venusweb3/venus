...
import { TeeLogQuery, TeeLogService } from "@venusos/plugin-tee-log";
import { REST, Routes } from "discord.js";
import { DirectClient } from ".";
import { validateUuid } from "@venusos/core";
...
export function createApiRouter(
    ...
    router.post("/agents/:agentId/set", async (req, res) => {
        ...
        try {
            ...
            venusLogger.error(`Error parsing character: ${e}`);
            res.status(400).json({
                success: false,
                message: e.message,
            });
        ...
        try {
            ...
            venusLogger.log(`${character.name} started`);
        } catch (e) {
            venusLogger.error(`Error starting agent: ${e}`);
            res.status(500).json({
                success: false,
                message: e.message,
            });
    ...
    router.get("/tee/agents", async (req, res) => {
        try {
            ...
            venusLogger.error("Failed to get TEE agents:", error);
            res.status(500).json({
    ...
    router.get("/tee/agents/:agentId", async (req, res) => {
        try {
            ...
            venusLogger.error("Failed to get TEE agent:", error);
            res.status(500).json({
    ...
    router.post(
        ...
        async (req: express.Request, res: express.Response) => {
            try {
                ...
                venusLogger.error("Failed to get TEE logs:", error);
                res.status(500).json({
