...
export class DirectClient {
    ...
    constructor() {
        venusLogger.log("DirectClient constructor");
        this.app = express();
        this.app.use(cors());
        this.agents = new Map();
        ...
        this.app.get(
            ...
            async (req: express.Request, res: express.Response) => {
                ...
                venusLogger.log("Download directory:", downloadDir);

                try {
                    venusLogger.log("Creating directory...");
                    await fs.promises.mkdir(downloadDir, { recursive: true });

                    venusLogger.log("Fetching file...");
                    ...
                    venusLogger.log("Response headers:", fileResponse.headers);
                    ...
                    venusLogger.log("Saving as:", fileName);

                    const arrayBuffer = await fileResponse.arrayBuffer();
                    const buffer = Buffer.from(arrayBuffer);

                    const filePath = path.join(downloadDir, fileName);
                    venusLogger.log("Full file path:", filePath);
                    ...
                    venusLogger.error("Detailed error:", error);
                    res.status(500).json({
    ...
    public start(port: number) {
        ...
        const gracefulShutdown = () => {
            venusLogger.log("Received shutdown signal, closing server...");
            this.server.close(() => {
                venusLogger.success("Server closed successfully");
                process.exit(0);
            });
    ...
    public stop() {
        if (this.server) {
            this.server.close(() => {
                venusLogger.success("Server stopped");
            });
        }
    }
}
...
export const DirectClientInterface: Client = {
    start: async (_runtime: IAgentRuntime) => {
        venusLogger.log("DirectClientInterface start");
