...
async function handleStreamResponse(
    ...
    venusLogger.log("Starting stream response handling");
    const reader = response.body?.getReader();
    if (!reader) {
        venusLogger.error("No readable stream available");
        ...
        async start(controller) {
            try {
                while (true) {
                    ...
                    if (done) {
                        venusLogger.log("Stream reading completed");
                        break;
                    }
                ...
                reader.releaseLock();
                controller.close();
                venusLogger.log("Stream controller closed");
            }
        },
    });
}
...
export const blockchainChatAction: Action = {
    ...
    handler: async (
        ...
        try {
            venusLogger.log("Starting blockchain chat handler");
            const secretKey =
                runtime.getSetting("THIRDWEB_SECRET_KEY") ??
                process.env.THIRDWEB_SECRET_KEY;

            if (!secretKey) {
                venusLogger.error("THIRDWEB_SECRET_KEY not configured");
            ...
            venusLogger.log("NEBULA CHAT REQUEST: ", request);

            venusLogger.debug("Sending request to Nebula API");
            ...
            venusLogger.debug("Received response from Nebula API");

            if (!request.stream) {
                const text = await response.text();
                venusLogger.debug("Raw response text:", text);

                try {
                    const cleanedText = text.trim().split("\n").pop() || text;
                    const parsed = JSON.parse(cleanedText);
                    venusLogger.log("Successfully parsed response:", parsed);
                    ...
                    venusLogger.error("Parse error details:", parseError);
                    venusLogger.error(
            ...
            venusLogger.log("Handling streaming response");
            return handleStreamResponse(response);
        } catch (error) {
            venusLogger.error("Blockchain chat failed:", error);
