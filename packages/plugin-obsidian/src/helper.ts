import { IAgentRuntime, AgentRuntime, ModelClass, Memory, MemoryManager } from "@venusos/core";
import { venusLogger, composeContext, generateObject, stringToUuid } from "@venusos/core";
...
export async function getObsidian(runtime: IAgentRuntime): Promise<ObsidianProvider> {
    if (!obsidianInstance) {
        venusLogger.debug("Creating new ObsidianProvider instance");
...
export async function storeHierarchyInMemory(runtime: IAgentRuntime, message: Memory, hierarchy: NoteHierarchy) {
    ...
    venusLogger.info(`Stored hierarchy for note ${hierarchy.path} in memory`);
}
...
export async function retrieveHierarchyFromMemory(runtime: IAgentRuntime, message: Memory, notePath: string): Promise<NoteHierarchy | null> {
    ...
    try {
        ...
        if (memories && memories.length > 0) {
            ...
            venusLogger.info(`Retrieved hierarchy for note ${notePath} from memory`);
            return hierarchy;
        }
        return null;
    } catch (error) {
        venusLogger.error(`Failed to retrieve hierarchy from memory: ${error.message}`);
        return null;
    }
}
...
async function genereteSearchParameters(prompt: string, state: any, runtime: IAgentRuntime): Promise<string> {
    try {
        ...
        venusLogger.info("Parsed completion:", JSON.stringify(parsedCompletion , null, 2));
