...
export const searchAction: Action = {
    ...
    handler: async (
        ...
        venusLogger.info("Starting search handler");
        const obsidian = await getObsidian(runtime);
        ...
        try {
            ...
            venusLogger.debug("Search context:", JSON.stringify(searchContext.query, null, 2));
            ...
            venusLogger.info(`Searching vault with ${queryFormat} query: ${typeof query === 'string' ? query : JSON.stringify(query)}`);
            ...
            venusLogger.info(`Found ${results.length} matching notes`);
...
#### ✅ ${result.filename} (**Score:** ${result.score})\n${matches}`;
                ...
                venusLogger.info("Formatted results:", formattedResults);
...
#### ✅ ${result.filename}`;
            ...
            venusLogger.error("Error searching vault:", error);
            if (callback) {
                callback({
