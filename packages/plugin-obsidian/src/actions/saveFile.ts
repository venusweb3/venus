...
export const saveFileAction: Action = {
    ...
    handler: async (
        ...
        venusLogger.info("Starting save file handler");
        const obsidian = await getObsidian(runtime);
        ...
        try {
            ...
            if (!content) {
                venusLogger.error("File content is required for saving");
                if (callback) {
                    callback({
            ...
            venusLogger.info(`Saving file at path: ${path}`);
            ...
            venusLogger.info(`Successfully saved file: ${path}`);
            ...
            venusLogger.error("Error saving file:", error);
            if (callback) {
                callback({
