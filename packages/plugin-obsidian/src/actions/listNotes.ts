...
export const listNotesAction: Action = {
    ...
    handler: async (
        ...
        venusLogger.info("Starting list notes handler");
        const obsidian = await getObsidian(runtime);

        try {
            venusLogger.info("Fetching list of notes from vault");
            const notes: string[] = await obsidian.listNotes();

            venusLogger.info(`Successfully retrieved ${notes.length} notes`);
            ...
            venusLogger.error("Error listing notes:", error);
            if (callback) {
                callback({
