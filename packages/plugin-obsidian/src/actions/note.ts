...
export const getNoteAction: Action = {
    ...
    handler: async (
        ...
        venusLogger.info("Starting get note handler");
        const obsidian = await getObsidian(runtime);
        ...
        try {
            ...
            venusLogger.info(`Fetching note at path: ${path}`);
            const noteContent: NoteContent = await obsidian.getNote(path);

            venusLogger.info(`Successfully retrieved note: ${path}`);
            ...
            venusLogger.error("Error retrieving note:", error);
            if (callback) {
                callback({
