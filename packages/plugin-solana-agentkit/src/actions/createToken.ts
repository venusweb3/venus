...
function isCreateTokenContent(content: any): content is CreateTokenContent {
    venusLogger.log("Content for createToken", content);
    return (
...
export default {
    ...
    handler: async (
        ...
        venusLogger.log("Starting CREATE_TOKEN handler...");
        ...
        if (!isCreateTokenContent(content)) {
            venusLogger.error("Invalid content for CREATE_TOKEN action.");
            if (callback) {
                callback({
        ...
        venusLogger.log("Init solana agent kit...");
        ...
        try {
            ...
            venusLogger.log("Create successful: ", deployedAddress);
            venusLogger.log(deployedAddress);
            if (callback) {
                callback({
            ...
            if (callback) {
                venusLogger.error("Error during create token: ", error);
                callback({
