...
NEAR Protocol integration plugin for venus OS that enables token management, transfers, and swaps using Ref Finance.
...
```bash
pnpm install @venusos/plugin-near
```
...
```typescript
const walletInfo = await venus.getProvider("wallet");
