...
export class InteractionClient {
    ...
    private async handleInteractions() {
        venusLogger.log("Checking EchoChambers interactions");
        ...
        try {
            ...
            venusLogger.log("Finished checking EchoChambers interactions");
        } catch (error) {
            venusLogger.error(
    ...
    private async handleMessage(message: ChatMessage, roomTopic: string) {
        try {
            venusLogger.debug("Processing message:", {
                id: message.id,
                room: message.roomId,
            ...
            venusLogger.debug("Converted IDs:", { roomId, userId });
            ...
            if (!response || !response.text) {
                venusLogger.log("No response generated");
                return;
            }
            ...
            venusLogger.error("Error handling message:", error);
            venusLogger.debug("Message that caused error:", {
                message,
                roomTopic,
            });
        }
    ...
    private async checkForDeadRooms() {
        try {
            ...
            for (const roomId of watchedRooms) {
                try {
                    venusLogger.debug(`Checking room ${roomId}`);

                    const room = rooms.find((r) => r.id === roomId);
                    if (!room) {
                        venusLogger.debug(`Room ${roomId} not found, skipping`);
                        continue;
                    }

                    // Log room details
                    venusLogger.debug("Room details:", {
                        id: room.id,
                        name: room.name,
                        topic: room.topic,
                    });
                    ...
                    venusLogger.error(`Error processing room ${roomId}:`, {
            ...
            venusLogger.debug("Full error details:", {
                error,
                stack: error?.stack,
                type: typeof error,
            });
    ...
    private async initiateConversation(room: ChatRoom) {
        try {
            venusLogger.debug(`Starting initiateConversation for ${room.name}`);
            ...
            venusLogger.debug("Composed state for conversation");
            ...
            venusLogger.debug("Created conversation context");
            ...
            venusLogger.debug("Generated response content:", {
                hasContent: !!content,
                textLength: content?.text?.length,
            });

            if (content?.text) {
                venusLogger.debug(`Sending message to ${room.name}`);
