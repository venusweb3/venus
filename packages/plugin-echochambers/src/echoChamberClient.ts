...
export class EchoChamberClient {
    ...
    public async addWatchedRoom(roomId: string): Promise<void> {
        try {
            ...
            venusLogger.success(`Now watching room: ${room.name}`);
        } catch (error) {
            venusLogger.error("Error adding watched room:", error);
            throw error;
        }
    }
    ...
    public removeWatchedRoom(roomId: string): void {
        ...
        venusLogger.success(`Stopped watching room: ${roomId}`);
    }
    ...
    private async retryOperation<T>(
        ...
        for (let i = 0; i < retries; i++) {
            try {
                ...
                venusLogger.warn(`Retrying operation in ${delay}ms...`);
    ...
    public async start(): Promise<void> {
        venusLogger.log("🚀 Starting EchoChamber client...");
        try {
            ...
            venusLogger.error("❌ Failed to start EchoChamber client:", error);
            throw error;
        }
    }
    ...
    public async stop(): Promise<void> {
        ...
        venusLogger.log("Stopping EchoChamber client...");
    }
    ...
    public async listRooms(tags?: string[]): Promise<ChatRoom[]> {
        try {
            ...
            venusLogger.error("Error listing rooms:", error);
            throw error;
        }
    }
    ...
    public async shouldInitiateConversation(room: ChatRoom): Promise<boolean> {
        try {
            ...
            venusLogger.error(`Error checking conversation state: ${error}`);
            return false;
        }
    }
}
