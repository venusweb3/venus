...
function isSwapContent(
    ...
    venusLogger.debug("Content for swap", content);
    return (
...
export default {
    ...
    handler: async (
        ...
        venusLogger.log("Starting SWAP_TOKEN handler...");
        ...
        if (!isSwapContent(runtime, content)) {
            venusLogger.error("Invalid content for SWAP_TOKEN action.");
            callback?.({
        ...
        venusLogger.debug("Swap content:", content);
        ...
        if (
            ...
            venusLogger.log("Swapping from native AVAX");
        } else if (
            content.toTokenAddress ===
            "0x0000000000000000000000000000000000000000"
        ) {
            // todo: swap to native
            venusLogger.log("Swapping to native AVAX");
        } else {
            ...
            if (tx) {
                ...
                if (receipt.status === "success") {
                    ...
                    if (swapTx) {
                        ...
                        if (receipt.status === "success") {
                            venusLogger.log("Swap successful");
                            callback?.({
                                text: "swap successful",
                                content: { success: true, txHash: swapTx },
                            });
                        } else {
                            venusLogger.error("Swap failed");
                            callback?.({
                                text: "swap failed",
                    ...
                    venusLogger.error("Approve failed");
                    callback?.({
                        text: "approve failed",
