...
export const postTweetAction: Action = {
    ...
    handler: async (
        ...
        if (
            ...
            venusLogger.error(
                `Parameter 'VERIFIABLE_INFERENCE_ENABLED' not set, venus will run this action!`
            );
            return false;
        }
        ...
        try {
            ...
            // post the tweet
            venusLogger.log("Attempting to send tweet:", contentYouWantToPost);
            const result = await scraperWithPrimus.sendTweet(contentYouWantToPost);

            venusLogger.log("Tweet response:", result);

            // Check for Twitter API errors
            if (!result) {
                venusLogger.error(`Twitter API error ${result}`);
                return false;
            }
            return true;
        } catch (error) {
