...
export class TelegramClient {
    ...
    constructor(runtime: IAgentRuntime, botToken: string) {
        venusLogger.log("📱 Constructing new TelegramClient...");
        this.options = {
            telegram: {
        ...
        venusLogger.log("✅ TelegramClient constructor completed");
    }

    public async start(): Promise<void> {
        venusLogger.log("🚀 Starting Telegram bot...");
        try {
            await this.initializeBot();
            this.setupMessageHandlers();
            this.setupShutdownHandlers();
        } catch (error) {
            venusLogger.error("❌ Failed to launch Telegram bot:", error);
            throw error;
        }
    }
    ...
    private async initializeBot(): Promise<void> {
        ...
        venusLogger.success(`Bot username: @${botInfo.username}`);
    ...
    private async isGroupAuthorized(ctx: Context): Promise<boolean> {
        ...
        if (!allowedGroups.includes(currentGroupId)) {
            venusLogger.info(`Unauthorized group detected: ${currentGroupId}`);
            try {
    ...
    private setupMessageHandlers(): void {
        venusLogger.log("Setting up message handler...");
        ...
        this.bot.on(message("new_chat_members"), async (ctx) => {
            try {
                ...
                venusLogger.error("Error handling new chat members:", error);
            }
        });
        ...
        this.bot.catch((err, ctx) => {
            venusLogger.error(`❌ Telegram Error for ${ctx.updateType}:`, err);
    ...
    private setupShutdownHandlers(): void {
        const shutdownHandler = async (signal: string) => {
            ...
            try {
                await this.stop();
                venusLogger.log("🛑 Telegram bot stopped gracefully");
            } catch (error) {
                venusLogger.error(
    ...
    public async stop(): Promise<void> {
        venusLogger.log("Stopping Telegram bot...");
        //await 
            this.bot.stop();
        venusLogger.log("Telegram bot stopped");
    }
}
