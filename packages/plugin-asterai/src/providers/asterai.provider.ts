...
const asteraiProvider: Provider = {
    get: async (
        ...
        if (!asteraiClient) {
            venusLogger.error("asteraiClient is not initialised");
            return null;
        }
        ...
        if (!agentSummaryMemory) {
            ...
            venusLogger.debug("asterai agent summary fetched:", summary);
        ...
        if (!agentSummaryMemory) {
            venusLogger.error("failed to initialise agent's summary memory");
            return null;
        }
