...
import { generateObjectDeprecated } from "@venusos/core";
import {
    Account,
    Aptos,
    AptosConfig,
...
function isTransferContent(content: any): content is TransferContent {
    venusLogger.debug("Validating transfer content:", content);
    return (
...
export default {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        venusLogger.debug("Starting transfer validation for user:", message.userId);
        venusLogger.debug("Message text:", message.content?.text);
    ...
    handler: async (
        ...
        venusLogger.debug("Starting TRANSFER_MOVE handler...");
        venusLogger.debug("Message:", {
        ...
        try {
            ...
            venusLogger.debug("Got private key:", privateKey ? "Present" : "Missing");

            const network = runtime.getSetting("MOVEMENT_NETWORK");
            venusLogger.debug("Network config:", network);
            venusLogger.debug("Available networks:", Object.keys(MOVEMENT_NETWORK_CONFIG));
            ...
            venusLogger.debug("Created Movement account:", movementAccount.accountAddress.toStringLong());
            ...
            venusLogger.debug("Created Aptos client with network:", MOVEMENT_NETWORK_CONFIG[network].fullnode);
            ...
            venusLogger.debug("Transfer successful:", {
