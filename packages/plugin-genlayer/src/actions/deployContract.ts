...
export const deployContractAction: Action = {
    ...
    handler: async (
        ...
        venusLogger.info("Starting deploy contract action");
        ...
        if (!options) {
            venusLogger.error("Failed to parse deploy contract parameters");
            throw new Error("Failed to parse deploy contract parameters");
        }

        venusLogger.debug("Parsed parameters:", options);
        venusLogger.info(
