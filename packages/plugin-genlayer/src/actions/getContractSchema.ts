...
export const getContractSchemaAction: Action = {
    ...
    handler: async (
        ...
        venusLogger.info("Starting get contract schema action");
        ...
        if (!addressMatch) {
            venusLogger.error("No valid address found in message");
        ...
        venusLogger.success("Successfully retrieved contract schema");
        venusLogger.debug("Contract schema:", result);
        await callback(
            {
