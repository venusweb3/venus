...
export const waitForTransactionReceiptAction: Action = {
    ...
    handler: async (
        ...
        venusLogger.info("Starting wait for transaction receipt action");
        venusLogger.debug("User message:", message.content.text);
        ...
        if (!hashMatch) {
            venusLogger.error("No valid transaction hash found in message");
            throw new Error("No valid transaction hash found in message");
        }

        venusLogger.info(`Waiting for receipt of transaction: ${hashMatch[0]}`);
        const result = await clientProvider.client.waitForTransactionReceipt({
            hash: hashMatch[0] as TransactionHash,
        });

        venusLogger.success("Successfully received transaction receipt");
        venusLogger.debug("Transaction receipt:", result);
        await callback(
            {
