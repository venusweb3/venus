...
function isLaunchAgentContent(content: any): content is LaunchAgentContent {
    venusLogger.log("Content for launchAgent", content);
...
export default {
    name: "LAUNCH_AGENT",
    similes: ["CREATE_AGENT", "DEPLOY_AGENT", "DEPLOY_venus", "DEPLOY_BOT"],
    validate: async (_runtime: IAgentRuntime, _message: Memory) => {
        return true;
    },
    description: "Launch an venus agent",
    handler: async (
        ...
        venusLogger.log("Starting LAUNCH_AGENT handler...");
        ...
        if (!isLaunchAgentContent(content)) {
            venusLogger.error("Invalid launch content", content);
            if (callback) {
                callback({
        ...
        try {
            ...
            if (callback) {
                venusLogger.error("Error during launching agent");
