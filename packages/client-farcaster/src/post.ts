...
export class FarcasterPostManager {
    ...
    constructor(
        ...
        venusLogger.log("Farcaster Client Configuration:");
        venusLogger.log(`- FID: ${this.fid}`);
        venusLogger.log(
    ...
    public async start() {
        const generateNewCastLoop = async () => {
            ...
            venusLogger.log(`Next cast scheduled in ${randomMinutes} minutes`);
        };
    ...
    private async generateNewCast() {
        venusLogger.info("Generating new cast");
        try {
            ...
            if (this.runtime.getSetting("FARCASTER_DRY_RUN") === "true") {
                venusLogger.info(`Dry run: would have cast: ${content}`);
                return;
            }
            ...
            try {
                ...
                venusLogger.error("Error sending cast:", error);
            }
        } catch (error) {
            venusLogger.error("Error generating new cast:", error);
        }
    }
}
