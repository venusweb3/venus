...
export class FarcasterInteractionManager {
    ...
    private async handleInteractions() {
        ...
        if (!agentFid) {
            venusLogger.info("No FID found, skipping interactions");
            return;
        }
    ...
    private async handleCast({
        ...
        if (cast.profile.fid === agent.fid) {
            venusLogger.info("skipping cast from bot itself", cast.hash);
            return;
        }

        if (!memory.content.text) {
            venusLogger.info("skipping cast with no text", cast.hash);
        ...
        const callback: HandlerCallback = async (
            ...
            try {
                ...
                venusLogger.error("Error sending response cast:", error);
                return [];
            }
        };
