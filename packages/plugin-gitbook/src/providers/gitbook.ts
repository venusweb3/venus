...
async function validateQuery(
    ...
    try {
        ...
        venusLogger.info(`✅ Is GitBook Validation Result: ${isValid}`);
        return isValid;
    } catch (error) {
        venusLogger.warn(`❌ Error in GitBook validation:\n${error}`);
        return false;
    }
}
...
export const gitbookProvider: Provider = {
    get: async (
        ...
        try {
            ...
            if (!spaceId) {
                venusLogger.error("⚠️ GitBook Space ID not configured");
                return "";
            }
            ...
            if (!isValidQuery) {
                venusLogger.info("⚠️ GitBook Query validation failed");
                return "";
            }
            ...
            if (!response.ok) {
                venusLogger.error("❌ GitBook API error:", response.status);
                return "";
            }
            ...
            venusLogger.error("❌ Error in GitBook provider:", error);
            return "";
        }
    },
};
