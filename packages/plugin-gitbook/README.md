# @venusos/plugin-gitbook

A plugin for querying and retrieving information from GitBook documentation within the venusOS ecosystem.
...
```bash
pnpm install @venusos/plugin-gitbook
```
...
```typescript
import { gitbookPlugin } from "@venusos/plugin-gitbook";
```
